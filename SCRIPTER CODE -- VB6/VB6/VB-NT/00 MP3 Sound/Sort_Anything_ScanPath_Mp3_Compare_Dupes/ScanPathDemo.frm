...
tpath3$ = "E:\04 Music ---\05 Whole Lot\"
tpath3$ = "E:\04 Music ---\03 My Music Zen\03 Small'S\Smalls Tv Theme'S\themes\"
...
Onfilenamechk = False ' do on compare
'Onfilenamechk = True 'do on dupe filename
...
For we = ListView1.ListItems.Count To 1 Step -1
    ...
    If Onfilenamechk = False Then ListView1.ListItems.Item(we).SubItems(1) = Format$(tt1, "0000000000") + " - " + a1$ + b1$
...
If Onfilenamechk = True Then
    ii1$ = LCase(b1$): ii2$ = LCase$(b2$)
End If
...
If aki = 0 Then Label14 = "No Dupes"
...
mdlID3.ReadMPEGInfo a1$ + b1$, outInfo
    ...
    kkl1 = outInfo.Bitrate
    
mdlID3.ReadMPEGInfo tpath2$ + c1$ + b1$, outInfo

    'lblkBit2 = "Bitrate: " & outInfo.Bitrate & " kbit / sec, " & IIf(outInfo.HasVBR, "variable bit rate", "constant bit rate")
    kkl2 = outInfo.Bitrate
...
fs22.copyFile a1$ + b1$, tpath2$ + c1$ + b1$
End If
If tt2 < tt1 And tt2 > 0 And kkl1 = kkl2 Then
snot2 = snot2 + 1
Label18 = Str(snot2)
...
ScanPath.Width = List1.Width + 210
