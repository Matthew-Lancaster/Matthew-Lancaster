...
Private Sub Form_Load()
    ...
    Next
    'VoiceCB.ListIndex = 3
    Open App.Path + "\VoiceCB.txt" For Input As #1
    Line Input #1, A$
    Close #1
    VoiceCB.ListIndex = Val(A$)
...
'    RateSldr.Value = Voice.Rate
    Open App.Path + "\VoiceRateSldr.txt" For Input As #1
    Line Input #1, A$
    Close #1
    RateSldr.Value = Val(A$)
...
'    FormatCB.Text = DefaultFmt
    Open App.Path + "\VoiceAudioOutPutCB.txt" For Input As #1
    Line Input #1, A$
    Close #1
    FormatCB.Text = A$
    ...
    'MainTxtBox = Format$(Now, "DDDD dd-MMMM-YY HH:MM:SSam/pm")
    MainTxtBox = Format$(Now, "DDDD dd MMMM")
...
Private Sub Form_Unload(Cancel As Integer)
    'If TrueTerminate = False Then Cancel = True: TTSAppMain.Visible = False: Exit Sub
...
Private Sub FormatCB_Click()
    ...
    Open App.Path + "\VoiceAudioOutPutCB.txt" For Output As #1
    Print #1, FormatCB.List(FormatCB.ListIndex)
    Close #1
...
Private Sub RateSldr_Scroll()
    Voice.Rate = RateSldr.Value
    Open App.Path + "\VoiceRateSldr.txt" For Output As #1
    Print #1, Str(RateSldr.Value)
    Close #1
...
Private Sub VoiceCB_Click()
    ...
    Open App.Path + "\VoiceCB.txt" For Output As #1
    Print #1, Str(VoiceCB.ListIndex)
    Close #1
End Sub
