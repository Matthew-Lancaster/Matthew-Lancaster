...
Private Sub Form_Load()
    ...
    If App.PrevInstance = True Then
        Open App.Path + "\Delay.txt" For Output As #1
        Close #1
        End
    End If
    
    If IsIDE Then Form1.Show
...
'    Call Extract_Work_UNet_Folder_Click2
'    Exit Sub
...
Dim oNameSpace1 As NameSpace
Dim oNameSpace2 As NameSpace
Dim oMailitem As Object
Dim sMessage As String
Dim oFolder1 As MAPIFolder
...
If oParentFolder.Folders.Count Then
  For r = 1 To oParentFolder.Folders.Count
    If Trim(oParentFolder.Folders(r).Name) = "00 @slv3Web Reports" Then i1 = r
    If Trim(oParentFolder.Folders(r).Name) = "00 @slv3Web Reports Done" Then i2 = r
    If i1 > 0 And i2 > 0 Then Exit For
  Next r
End If

Set oFolder1 = oParentFolder.Folders(i1) '00 @slv3Web Reports
tt1$ = oFolder1.Name

Set oFolder2 = oParentFolder.Folders(i2) '00 @slv3Web Reports Done
tt2$ = oFolder2.Name
...
'Set MyItems4 = myolApp1.CreateItem(olMailItem) 'Just Dummy
...
Dim exCnt As Integer, TT As String, EE As String
...
'For rrr = 1 To oFolder1.Items.Count
Do
    If oFolder1.Items.Count = 0 Then Exit Do
    Set oMailitem = oFolder1.Items(1)
        TT = oMailitem.Subject
        qq = qq + 1
        If InStr(TT, "/var/www/vhosts/matthewlan.com/statistics/logs/access_log.processed.") Then
                    
        TD = oMailitem.ReceivedTime
        EE = oMailitem.Body
        file1 = "MyLogg--" + Format$(ts, "YYYY-MM")
        yy = InStr(EE, " - - [")
        tf1 = DateValue(Mid$(EE, yy + 6, 11))
        yy = Year(tf1)
        mm = Month(tf1) + 1
        If mm = 13 Then yy = yy + 1: mm = 1
        tf2 = DateSerial(yy, mm, 1)
        path1 = "D:\MY WEBS\MatthewLan.Com Web\awstats2\var\"
        file1 = path1 + "MyLogg--" + Format$(tf1, "YYYY-MM-01") + "--to--" + Format$(tf2, "YYYY-MM-DD") + "--.log"
        ...
        'lstExtractionStatus.AddItem Str(qq) + " of" + Str(rr)
...
If Dir$(App.Path + "\Delay.txt") <> "" Then
        Enow = Now + TimeSerial(0, 1, 0)
        Kill App.Path + "\Delay.txt"
End If

If Enow = 0 Then Enow = Now + TimeSerial(0, 1, 0)
'If enow < Now Or IsIDE = True Then
If Enow < Now And Enow > 0 Then
    Enow = -1
    Call WebReports
    End
End If
