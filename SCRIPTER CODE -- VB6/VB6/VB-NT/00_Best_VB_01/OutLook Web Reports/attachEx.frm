...
Do
    ...
    mq = mm - 1
    If mq = 0 Then yy = yy - 1: mq = 1
    tf5 = tf1
    tf1 = DateSerial(yy, mq, 1)
    tf2 = DateSerial(yy, mm, 1)
    path1 = "D:\MY WEBS\MatthewLan.Com Web\awstats2\var\"
    file1 = path1 + "MyErrorLogg--" + Format$(tf1, "YYYY-MM-01") + "--to--" + Format$(tf2, "YYYY-MM-DD") + "--.log"
Loop Until Dir$(file1) = ""
tf3 = tf5
...
Do
    ...
    Set oMailitem = oFolder1.Items(1)
        ...
        If InStr(TT, "/var/www/vhosts/matthewlan.com/statistics/logs/access_log.processed.") Then
            ...
            If InStr(milky, Mid$(EE, 1, 300)) = 0 Then
            Open file1 For Append As #1
                If IsIDE = False Then Print #1, EE
            Close #1
            End If
        ...
        If InStr(TT, "/var/www/vhosts/matthewlan.com/statistics/logs/error_log.") Then
            TD = oMailitem.ReceivedTime
            EE = oMailitem.Body
            yy1 = InStr(EE, "[error]")
            yy2 = InStr(EE, "[warn]")
            yy = 0
            If yy1 > 0 And yy2 > 0 Then If yy1 > yy2 Then yy = yy2 Else yy = yy1
            If yy1 = 0 And yy2 > 0 Then yy = yy2
            If yy1 > 0 And yy2 = 0 Then yy = yy1
            If Mid$(EE, 1, 1) = "[" Then
                yy = 28
            End If
            On Error Resume Next
                tfvar = tf1
                Err.Clear
                tf1 = DateValue(Mid$(EE, yy - 22, 7) + Mid$(EE, yy - 6, 4))
                If Err.Number > 0 Then
                    If tfvar = 0 Then tf1 = tf3
                End If
            On Error GoTo 0
            yy = Year(tf1)
            mm = Month(tf1) + 1
