Attribute VB_Name = "PID_Hwnd_Module"
Option Explicit
...
Private Function ExecCmdWAIT(cmdLine$) As Long
    Dim proc As PROCESS_INFORMATION
    Dim start As STARTUPINFO
    Dim Ret&
...
Public Function ExecCmdWAIT_IS_SHOWING(cmdLine$, VAR_SHOW_HIDE) As Long
    Dim proc As PROCESS_INFORMATION
    Dim start As STARTUPINFO
    Dim Ret&
    '---------------------------
    'BETTER OF THE TWO SHELLWAIT
    '---------------------------
    'SET PRIORITY
    '---------------------------
    'RETURN THE HWND
    '---------------------------
    'LOADS OF SETTING IN STARTUPINFO
    '---------------------------


     ' Initialize the STARTUPINFO structure:
    start.wShowWindow = vbNormal
    start.wShowWindow = VAR_SHOW_HIDE
    start.cb = Len(start)
    start.wShowWindow = VAR_SHOW_HIDE
    start.cb = Len(start)
...
'    Call WaitForSingleObject(proc.hProcess, INFINITE)
    ...
    Call WaitForSingleObject(proc.hProcess, 1000)
    Dim HWND_RETURN As Long
    
    If Ret Then
       'ExecCmdWAIT_IS_SHOWING = InstanceToWnd(proc.dwProcessId)
       'Me.Print Ret, ExecCmd
        'ExecCmdWAIT_IS_SHOWING = GetHWnd(proc.dwProcessId, HWND_RETURN)
        
        ExecCmdWAIT_IS_SHOWING = cProcesses.Convert(HWND_RETURN, proc.dwProcessId, cnFromProcessID Or cnTohWnd)
