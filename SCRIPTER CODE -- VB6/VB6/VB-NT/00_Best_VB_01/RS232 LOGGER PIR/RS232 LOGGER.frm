...
If VAR_DSR_4 = True Then
    For R = 1 To UBound(AR)
        ...
        ' DD = "C:\SCRIPTOR DATA\VB6\VB-NT\00_Best_VB_01\Tidal_Info\RS232 FRONT DOOR.txt"
        ...
        FILE_NAME_CHK = "C:\SCRIPTOR DATA\VB6\VB-NT\00_BEST_VB_01\TIDAL_INFO\RS232 FRONT DOOR_REBOOT__#NFS_EX.TXT"
        
        If Date <> "06/06/2020" Then
        ' GONNA REQUIRE DEBUG HERE
        If IsIDE = True Then Stop
        End If
        
        If FSO.FILEExists(FILE_NAME_CHK) = True Then
            Kill FILE_NAME_CHK
            FIRST_BOOT_CODE = False
            Me.Hide
            REBOOT_METHOD_DONE = True
        End If
        
        If REBOOT_METHOD_DONE = True Then
        ...
        End If
        ' -----------------------------------------------
        ' WORK HERE PROVIDE RESET METHOD EVERY 24 HOUR
        ' 4TH PROJECT TODAY
        ' MAKE RS232 RESET MIDNIGHT
        ' AND NOT COMPLAIN BEEN SHUT DOWN PROPER
        ' NEAR DONE WORK WRITE FOR VB2008 ALSO
        ...
        If InStr(UCase(STATE_DOOR), "OPEN") > 0 Then
            If FIRST_BOOT_CODE = True Then
                ...
                If FSO.FILEExists(FILE_NAME) = True Then
                    Me.WindowState = vbNormal
                    IR = MsgBox("FILE EXIST FOR FIRST RUN CODE" + vbCrLf + "CONFIRM OTHER CODE NOT LEFT BY ACCIDENT" + vbCrLf + "MIGHT NOT RUN CORRECT" + vbCrLf + "REMAIN OR LEAVE DELETE -- YES OR NOT" + vbCrLf + vbCrLf + "REMAIN WILL MAKE DOOR GO OPEN AGAIN", vbYesNo + vbMsgBoxSetForeground)
                    If IR = vbNo Then
                        Kill FileName
                        KILL_DONE = True
                        Me.Hide
            ...
            If InStr(UCase(STATE_DOOR), "OPEN") > 0 And KILL_DONE = False Then
                ' -----------------------------------------------
                ' NOT SURE IF FILE -- FILE_NAME_2 AND FILE_NAME_8
                ' REQUIRE DELETER AS ABOVE ONLY FILE_NAME_4 __ DO
                ' -----------------------------------------------
