...
Public Function GetWindowState(ByVal lngHwnd As Long) As Integer
    'MsgBox Str(IsWindow(lngHwnd)) + " -- " + Str(IsIconic(lngHwnd)) + " -- " + Str(IsZoomed(lngHwnd)) + " -- "
    ...
    'STILL PROBLEM RECT DON'T SHOW INFO OF MINIMIZED
'    Dim RECTT As RECT
'    HwndT = GetWindowRect(lngHwnd, RECTT)
'    Form2.Show
'    Form2.Label1.Caption = Str(RECTT.Top) + Str(RECTT.Left) + Str(RECTT.Right) + Str(RECTT.Bottom)
...
'FROM PROGRAM MAKE PATH ON ANOTHER DRIVE
...
Function Parent_Class_Text_Match(ByVal target_HWND As Long, ByVal target_Class As String) As Long
    Dim test_hwnd As Long, _
        test_pid As Long, _
        test_id As Long
    ...
    Do While test_hwnd <> 0
       ...
       If GetParent(test_hwnd) <> 0 Then
         ...
         If GetWindowClass(test_hwnd) = target_Class Then
            Parent_Class_Text_Match = test_hwnd
            Stop
            Exit Do
         End If
       End If
