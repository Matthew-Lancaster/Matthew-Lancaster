...
'Public CHD, MHD, NHD
Public LK2, OldDate2 ', FreeMem, IIO2, IIO3
...
Next
On Error GoTo 0
'Counter2.Width = wc + 90 + hq
'Counter2.Height = hc + 380
Counter2.Width = WC + hq
Counter2.Height = HC




If Dir$(App.Path + "\00_Text_Data\" + GetComputerName + "-" + GetUserName + "--\00 Knocker Boy2.txt") <> "" Then
    Open App.Path + "\00_Text_Data\" + GetComputerName + "-" + GetUserName + "--\00 Knocker Boy2.txt" For Input As #1
    Do
    Line Input #1, R2$
...
TTCount3 = Val(Mid$(R2$, 1, 4))
OldDate2 = DateValue(Mid$(R2$, 16, 9)) + TimeValue(Mid$(R2$, 26, 9))


'If r2$ <> "" Then
    'TTCount3 = Val(Mid$(r2$, 1, 4))
    'OldDate2 = DateValue(Mid$(r2$, 16, 9))
    TTCount4 = Val(Mid$(R2$, 6, 5))
    'If Int(Now) <> DateValue(Mid$(r2$, 16, 9)) Then TTCount3 = 0: OldDate2 = Int(Now)
'End If

Label10.Caption = Trim(str(TTCount3))
...
Public Sub WriteCounter()
    ...
    If OldDate2 < xdate Then
        TTCount3 = 0
    End If

    OldDate2 = Now
    
    FILENAME_IN_USE_CHECK = App.Path + "\00_Text_Data\" + GetComputerName + "-" + GetUserName + "--\00 Knocker Boy2.txt"
    ...
    TTCount3 = TTCount3 + 1
    TTCount4 = TTCount4 + 1
    Label10 = Trim(str(TTCount3))
    
    If Menu.MuteAllMal = vbChecked Then fg$ = fg$ + " -- Mute All Mal"

    QQ$ = Format$(TTCount3, "0000 ") + Format$(TTCount4, "00000-") + Format$(Now, "DDD-DD-MMM-YY HH:MM:SSa/p ") + "-" + fg$
    'List1.AddItem qq$
    ...
    Counter2.BackColor = Label5.BackColor
    Timer4.Enabled = True
...
hwndw = FindWindow(vbNullString, Counter1.Caption)
If hwndw = 0 Then Exit Sub
Dim Rect4 As RECT
Hwnd24 = GetWindowRect(hwndw, Rect4)
On Local Error Resume Next
If Ik2 <> Rect4.Top Then
    Counter2.Top = (Rect4.Bottom * Screen.TwipsPerPixelX)
    Counter2.Left = (Counter1.Left) '- Counter.Width)
End If
Ik2 = Rect4.Top
End Sub
...
Counter2.BackColor = Label6.BackColor
