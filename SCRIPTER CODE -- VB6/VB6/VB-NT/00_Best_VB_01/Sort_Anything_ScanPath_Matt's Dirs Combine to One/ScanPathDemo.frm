...
G(1) = "S:\00 Art\00 My Pictures\01 All Phone"
'G(2) = "S:\00 Art\00 My Pictures\02 Me & Family & Friends"
G(1) = "D:\# MY DOCS\# 01 My Documents\Blue Tooth Exchange Folder\01 All Phone\00 Mental"
...
ListView1.SortKey = 1 'ListView1.ColumnHeader(1)
...
For we = 1 To ListView1.ListItems.Count
    ...
    uu$ = Mid$(G(r2), 1 + InStrRev(G(r2), "\", Len(G(r2)) - 1))
    uu$ = Left$(uu$, Len(uu$) - 1)
    MkDir "S:\All One Pic's\" + uu$
    On Local Error GoTo 0
'    ud$ = "S:\All One Pic's\" + uu$ + "\"
    ...
    If a1$ <> d2$ And InStr(qq$, WxHex$) = 0 Then
    we2 = we2 + 1
    cnt$ = Format$(we2, "000000") + " - "
    ud$ = "S:\All One Pic's\" + uu$ + "\"

    Fs22.copyFile a1$ + b1$, ud$ + cnt$ + b1$
    End If
    qq$ = qq$ + "- " + WxHex$
    ...
    Do
        If Err.Number > 0 Or rg > 0 Then
            DoEvents
            f1 = FreeFile
            Open a1$ + b1$ For Binary As #f1
            FV1$ = Space$(LOF(f1))
            Get f1, , FV1$
            Close #1
            f1 = FreeFile
            Open ud$ + b1$ For Binary As #f1
            FV2$ = Space$(LOF(f1))
            Get f1, , FV2$
            Close #1
...
'            If a1$ <> d2$ Or rg > 0 Or 1 = 1 Then
            If rg > 0 Or FV1$ <> FV2$ Then
                countt = countt + 1
...
Skip:
    ...
    If a1$ <> d2$ Or kk = 1 Then
        If kk = 0 Then
            List1.AddItem Format$(we, "000 ") + "Move > " + cnt$ + b1$ + "--- " + a1$
        Else
