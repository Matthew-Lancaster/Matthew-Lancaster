...
'        If we < ListView1.ListItems.Count Then we = we + 1
    End If
Next
End If
...
For we = 2 To ListView1.ListItems.Count
    ...
    Set ff1 = fs22.getfile(a1$ + b1$)
    Set ff2 = fs22.getfile(a12$ + b12$)
    ...
    If ff1.Size = 0 And ff2.Size = 0 Then
        qb = 1
    End If
    If b1$ <> b12$ And qb = 1 Then qb = 0
    If Trim(g1$) = "" Or Trim(h1$) = 0 And qb = 1 Then qb = 0
    f1$ = Mid$(a1$, InStrRev(a1$, "\", Len(a1$) - 1))
    f2$ = Mid$(a12$, InStrRev(a12$, "\", Len(a12$) - 1))
    If f1$ <> f2$ And qb = 1 Then
        qb = 0
    End If
