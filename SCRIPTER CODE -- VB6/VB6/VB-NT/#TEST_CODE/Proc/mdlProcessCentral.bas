...
Public Sub List_ActiveProcess()
    ...
    '    .Nodes.Clear
        ...
        Rf$ = GetFileFromProc(process(i).th32ProcessID)
        
        If Rf$ <> "" Then Ew$ = Rf$
        rr = ProcessID(i)
        'CID_Run_Me.List1.AddItem Format$(rr, "0000") + "." + Format$(O, "0000") + "." + Format$(i, "0000 ") + Ew$
'        ProcLB.List1.AddItem Format$(rr, "0000 ") + Module(i).szExePath + Ew$
        Cnt = Cnt + 1
        ProcLB.List1.AddItem Format$(Cnt, "0000 ") + "Module - " + Format$(rr, "0000 ") + Ew$
...
Public Sub List_ActiveModules()
    ...
    For O = 0 To pCount
        ...
        ProcLB.List1.AddItem Format$(Cnt, "0000 ") + "Process - " + Format$(rr, "0000 ") + Ew$
...
Public Sub Process_Low2(P_ID As Long)
...
Public Sub Thread_Suspend(T_ID As Long)
    ...
    On Error GoTo VB_Error
        ...
        If CloseHandle(hThread) = False Then A = A 'Err_Dll Err.LastDllError, "CloseThread failed", sLocation, "Suspend_Thread"
