...
Begin VB.Form frmMain 
   ...
   Begin VB.Menu Mnu_Menu 
      Caption         =   "Menu"
      Begin VB.Menu Mnu_IDTag 
         Caption         =   "Call Vb Id Taggs"
      End
      Begin VB.Menu Mnu_NiceNames 
         Caption         =   "Call VB Nice Names"
      End
      Begin VB.Menu Mnu_DummyTaggsWinamp 
         Caption         =   "Call Dummy Taggs For Winamp PlayList"
      End
   End
End
...
For we = ListView1.ListItems.Count To 1 Step -1
    ...
    'd1$ = Mid$(b1$, 1, Len(b1$) - 3) + "html"
    
    If FS.FileExists("D" + Mid$(a1$, 2) + b1$) = False Then
    
    FS.copyfile a1$ + b1$, "D" + Mid$(a1$, 2) + b1$
    End If
    'Name a1$ + b1$ As a1$ + d1$
...
'below has some nice track rename features put real song name in think
For we = ListView1.ListItems.Count To 1 Step -1
    a1$ = ListView1.ListItems.Item(we).SubItems(1)
    b1$ = ListView1.ListItems.Item(we)

    fg = 0: xx = 0
    If InStr(LCase(b1$), "- track ") Then fg = 1
    If fg = 0 Then ListView1.ListItems.Remove (we)
...
For we = ListView1.ListItems.Count To 1 Step -1
    ...
    b1$ = ListView1.ListItems.Item(we)
    d1$ = b1$
    ww = InStr(LCase(b1$), "- track ")
    fg = 0
    If Mid$(d1$, ww + 8, 1) = " " Then Mid$(d1$, ww + 8, 1) = "0": fg = 1
    'If Mid$(d1$, 8, 1) = "c" Then Mid$(d1$, 8, 1) = "1"
...
For ry = 1 To ff5
    For rx = 1 To ff5
        If ff$(ry) = ff$(rx) And ry <> rx Then

            MsgBox "Error Dupes Names " + vbCrLf + ff$(ry)
...
txtPath.Text = "E:\04 Music ---\03 My Music Zen"
Call cmdScan_Click
For we = ListView1.ListItems.Count To 1 Step -1
    DoEvents
    a1$ = ListView1.ListItems.Item(we).SubItems(1)
    ...
    pio = 0
    For ry = 1 To ff5
    If b1$ = ff$(ry) Then pio = 1: Exit For
    Next
    If pio = 0 Then
    ...
    Fs22.moveFile dd$ + b1$, a1$ + b1$
    If Err.Number > 0 Then MsgBox "Error" + vbCrLf + Err.Description + vbCrLf + a1$ + b1$
    On Local Error GoTo 0
    DoEvents
    End If
Next
Call Mnu_IDTag_Click
End Sub
...
Private Sub Mnu_DummyTaggsWinamp_Click()
Shell "C:\Program Files\Microsoft Visual Studio\VB98\vb6.exe ""D:\VB6\VB-NT\00 MP3\Sort_Anything_ScanPath_Mp3_Dummy_Id_Tag_For_Winamp_Playlist\Sort_Anything_ScanPath_Mp3_Dummy_Id_Tag_For_Winamp_Playlist.vbp""", vbNormalFocus

End Sub

Private Sub Mnu_IDTag_Click()
Shell "C:\Program Files\Microsoft Visual Studio\VB98\vb6.exe /r ""D:\VB6\VB-NT\MP3 Tags File Title Ace\MP3 Tags File Title ID3v2.vbp""", vbNormalFocus

End Sub

Private Sub Mnu_NiceNames_Click()
Shell "C:\Program Files\Microsoft Visual Studio\VB98\vb6.exe ""D:\VB6\VB-NT\MP3 Bangers Nice FileNames\MP3 Bangers Nice FileNames.vbp""", vbNormalFocus
