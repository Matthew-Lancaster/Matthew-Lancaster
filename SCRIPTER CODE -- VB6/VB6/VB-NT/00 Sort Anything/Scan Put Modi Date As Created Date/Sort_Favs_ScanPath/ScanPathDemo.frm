...
Public Dog, OutPutPath, Xdate As Date, Tdate As Date, Zdate$, Ydate As Date, W$
...
txtPath.Text = "M:\00 Blue Tooth Exchange Folder"
Call cmdScan_Click

txtPath.Text = "M:\0 00 Art\00 My Pictures"
Call cmdScan_Click
...
For we = ListView1.ListItems.Count To 1 Step -1
    ...
    d3 = 0
    d3 = Abs(DateDiff("s", d1, d2))
'    If d3 > 1 Then Stop
    If d3 < 2 Then d3 = 0
    If d3 > 0 Then If d1 < d2 Then d3 = d1 Else d3 = d2
    If d1 > d2 Then d3 = 0
    If d3 > 0 Then
    
    Dum = SetFileDateTime(A1$ + B1$, d3)
    Else
