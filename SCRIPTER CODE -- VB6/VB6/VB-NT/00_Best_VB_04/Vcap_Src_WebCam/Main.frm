...
Begin VB.Form frmMain 
   ...
   Begin VB.Menu mnuFile 
      ...
      End
      Begin VB.Menu Mnu_Capture_Video_Seqence 
         Caption         =   "Capture Video Seqence"
      End
   ...
   Begin VB.Menu mnuCapture 
      ...
      End
   End
   Begin VB.Menu mnuCapVid_Quick_Capture_Video_Settings 
      Caption         =   "Q-Captue Video Setting"
   End
   Begin VB.Menu mnuCapVid2_Click 
      Caption         =   "&Quick Capture"
   End
   Begin VB.Menu Mnu_VideoFolder 
      Caption         =   "Open Video Folder D Drive"
   End
...
Private Sub Mnu_Capture_Video_Seqence_Click()
Mnu_Capture_Video_Seqence_Click_True = True
Load frmCapVid
frmCapVid.Show
'Mnu_Capture_Video_Seqence_Click_True = False
End Sub
...
Private Sub Mnu_VideoFolder_Click()
...
'Shell "Explorer.exe /select, " + SubPath, vbNormalFocus
Shell "Explorer.exe " + SubPath, vbNormalFocus
...
Private Sub mnuAllocFileSpace_Click()
    ...
    'TEMP OPTIONAL TAKE THIS OUT FOR AUTO
'    AllocWnd.Show vbModal, Me
    
'    Set AllocWnd = Nothing

    'USE THIS INSTEAD FOR AUTO
    
    Call capFileAlloc(frmMain.capwnd, "1" * ONE_MEGABYTE)
    'DONT SEEM TO WORK ALL FILES WERE ABOUT 400MB
...
Private Sub mnuCapture2_Click()

End Sub

Private Sub mnuCapVid_Click()
    
    Call mnuSetCapFile_Click
...
Private Sub mnuCapVid_Quick_Capture_Video_Settings_Click()
Load frmCapVid
frmCapVid.Show
...
Private Sub mnuCapVid2_Click_Click()
    
    QuickCapture = True
    'frmCapVid.chkLimit = vbChecked
    'frmCapVid.chkAudio = vbChecked
...
'frmAlloc.txtAlloc.Text = Val(1 * ONE_MEGABYTE)
frmAlloc.txtAlloc.Text = Str(Val(1 * ONE_MEGABYTE))

'CapFile = mVFW.capFileGetCaptureFile(hCapWnd)
'Exit Sub
'On Error Resume Next
'MkDir "M:\Videos"
'MkDir "M:\Videos\WebCam"
'On Error GoTo 0

'CapFile = "M:\Videos\WebCam\WebCam--" + Format(Now, "YYYY-MM-DD--HH-MM-SS") + ".avi"
CapFile = path
...
If True = retVal Then 'user did not cancel
    ...
    Else
        ...
        If Err.Number = 53 Then 'file does not exist
            'MsgBox "Err.Number = 53 file does not exist": Stop
