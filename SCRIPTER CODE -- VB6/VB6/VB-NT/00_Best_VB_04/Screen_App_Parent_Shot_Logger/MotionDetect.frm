...
CurProcHwnd = GetParentWindowHwnd(GetForegroundWindow)
...
hallo = GetWindowTitle(GetParentWindowHwnd(Mwnd))

'hallo = GetParentWindowTitle(Mwnd)
If hallo = "JPEG Encoder Demo" Then
End
End If
...
On Error Resume Next
TS3 = App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot ArrayPic.txt"
...
FF$ = "ScreenAppParentCapture_" + Format$(Now, "YYYY-MM-DD-DDD")
On Error Resume Next
MkDir "M\0 00 Art Loggers\Screen-App-Parent-Shots"
Err.Clear
MkDir "M\0 00 Art Loggers\Screen-App-Parent-Shots\" + FF$
If Err.Number = 76 Then End
On Error GoTo 0
FileInUseDelay App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot Pic Always.jpg"
SavePicture Form1.Picture, App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot Pic Always.jpg"
FileInUseDelay App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot % Pic Data.txt"
Open App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot % Pic Data.txt" For Append As #1
...
'If Wo / (Ri + Wo) * 100 >= 1 Then
    FileInUseDelay App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot ArrayPic.jpg"
    SavePicture Form1.Picture, App.Path + "\VBDataNoArchive\Screen-App-Parent-Shot ArrayPic.jpg"
    'TS1 = "M\0 00 Art Loggers\Screen-App-Shots\" + FF$ + "\ScreenAppCapture- " + Format$(Now, "YYYY-MM-DD HH-MM-SS DDD") + " - " + Format$(Wo / (Ri + Wo) * 100, "0") & "%.jpg"
    TS1 = "M\0 00 Art Loggers\Screen-App-Parent-Shots\" + FF$ + "\ScreenAppParentCapture- " + Format$(Now, "YYYY-MM-DD HH-MM-SS DDD") + ".jpg"
    FileInUseDelay TS1
    ...
    TS2 = "M\0 00 Art Loggers\Screen-App-Parent-Shots\" + FF$ + "\ScreenAppParentCapture- " + Format$(Now, "YYYY-MM-DD HH-MM-SS DDD") + ".txt"
    FileInUseDelay TS2
    Open TS2 For Output As #1
    Print #1, "---------------------"
    Print #1, "Screen Parent Application Shot FileName ="
    Print #1, TS1
    Print #1, "---------------------"
    Print #1, "Current Parent Handle Hwnd ="; CurProcHwnd
    Print #1, "---------------------"
    Print #1, "Current Handle Hwnd ="; CurProcHwnd
    Print #1, "---------------------"
    Print #1, "Application Parent Title = "; GetWindowTitle(CurProcHwnd)
    Print #1, "---------------------"
...
Public Function GetParentWindowHwnd(Test_Hwnd5 As Long) As String
   Dim i As Long
   Dim j As Long
   i = Test_Hwnd5
   If i Then
