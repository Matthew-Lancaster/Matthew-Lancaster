...
VBS_29_COPY_CAMERA_PHOTO_IMAGE_SOUND_PING_COUNTER=
...
TIMER_GOODSYNC_DELETE_PROCESS_VIA_VBS:
	...
	; ---------------------------------------------------------------
	; HERE WORK WITH HER
	; C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 79-GOODSYNC SCRIPT VB EXE.VBS
	; ---------------------------------------------------------------
	; AND HERE
	...
	; %A_ScriptDir%
	LINE_VAR=
	U2:="C:\SCRIPTOR DATA\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 79-GOODSYNC SCRIPT VB EXE_VBS_DATA_#NFS_EX.TXT"
	IF FILEEXIST(U2)
	{
		; LAST LINE WANT OF 2
		LOOP, read, %U2%
...
; TWO ROUTINE TOGETHER HERE AND ABOVE
; HERE WERE A TEST FOR FFMPEG
...
BAT_01_BOOT_KILLER_BAT_EXIST_KILL_TIMER:
	...
	IF BAT_01_BOOT_KILLER_BAT_EXIST_HWND<>%OLD_BAT_01_BOOT_KILLER_BAT_EXIST_HWND%
	{
		...
		SOUNDPLAY, %a_scriptDir%\Autokey -- 10-READ MOUSE CURSOR ICON\start.wav
		; MSGBOX "BAT_01_BOOT_KILLER_BAT_EXIST_KILL_TIMER 1111"
	}
	...
	IF BAT_01_BOOT_KILLER_BAT_EXIST_TIMER_COUNT_DOWN<%A_NOW%
	{
		...
		SOUNDPLAY, %a_scriptDir%\Autokey -- 10-READ MOUSE CURSOR ICON\start.wav
		; MSGBOX "BAT_01_BOOT_KILLER_BAT_EXIST_KILL_TIMER 2222"
...
NOT_RESPOND_CLIPBOARD_VIEWER_02:
	...
	RUN_EXE_GO_02:="D:\VB6\VB-NT\00_BEST_VB_01\CLIPBOARD_VIEWER\%VAR_1_WIN_02%.exe" 
	...
	IfWinNotExist %VAR_1_WIN_02%
	{
		...
		If Not ErrorLevel
		{
			if FileExist(RUN_EXE_GO_02)
			{
				RUN, %RUN_EXE_GO_02% /MINIMAL____START_22,MIN
				GOSUB SOUND_START_WAV
	...
	If Not ErrorLevel
		{
			...
			FN_VAR4:="C:\Program Files\Google\Drive File Stream\launch.bat"

			FN_VAR2:="C:\Program Files\Google\Drive File Stream\117.0.0.0\GoogleDriveFS.exe --crash_handler_token=\\.\pipe\crashpad_2944_WIAWOKEIBLKSLETZ --parent_version=117.0.0.0"
			IfExist, %FN_VAR4%
			{
				SOUNDPLAY, %a_scriptDir%\Autokey -- 10-READ MOUSE CURSOR ICON\start.wav
				Run, "%FN_VAR%"
...
TIMER_FILE_LOCATOR_GET_CONTROL__SET_FOCUS_ACTIVATE:
	...
	SOUNDPLAY, %a_scriptDir%\Autokey -- 10-READ MOUSE CURSOR ICON\start.wav
	; MSGBOX "TIMER_FILE_LOCATOR_GET_CONTROL__SET_FOCUS_ACTIVATE"
RETURN
		...
		; {
			...
			; {
				...
				; {
					...
					; Soundplay, "C:\SCRIPTER\SCRIPTER CODE -- AUTOHOTKEY\Autokey -- 10-READ MOUSE CURSOR ICON\star.wav"
...
HIGHER_SPEED:
	...
	IF OLD_id<>%id%
	{
		...
		IfWinActive ahk_class CabinetWClass
		{
			...
			{
				; ---------------------------------------------------
				; AHK MAX MAKE FULL SCREEN SIZE 
				; NOT WANT
				; MAXIMIZE OUGHT TO BE WITH TASK BAR AT BOTTOM
				; ---------------------------------------------------
				; WinMaximize, AHK_ID %ID%
				POSTMESSAGE, 0x112, 0xF030,,,AHK_ID %ID%  ; 0x112 = WM_SYSCOMMAND, 0xF030 = SC_MAXIMIZE
...
PARTITIONWIZARD_EXE_MAXIMIZE_VIDEO_WINDOW_SUB:
	...
	; ahk_exe partitionwizard.exe

	IF INSTR(ID_FOR_MAXIMUM_TITLE,"MiniTool Partition Wizard Free 12.1")>0 
	IF INSTR(ID_FOR_MAXIMUM_CLASS,"Qt5QWindowIcon")>0 
	{
		ID_STRING=%ACTIVE_ID% -- %ID_FOR_MAXIMUM_CLASS% -- %ID_FOR_MAXIMUM_TITLE% -- 
		IF INSTR(OLD_PARTITIONWIZARD_EXE_WINDOW_STR,ID_STRING)=0
		{
			; WINMAXIMIZE, AHK_ID %ACTIVE_ID%
			WINGET STYLE_PARTITIONWIZARD, MinMax, AHK_ID %ACTIVE_ID%
			IF STYLE_PARTITIONWIZARD<>1
			{
				; MAXIMIZE FULL SCREEN HAPPEN
				; WINDOW DOES NOT RESPOND CORRECTLY TO WINMAXIMIZE, TRY USING THE FOLLOWING INSTEAD
				POSTMESSAGE, 0x112, 0xF030,,,AHK_ID %ACTIVE_ID%  ; 0x112 = WM_SYSCOMMAND, 0xF030 = SC_MAXIMIZE
			...
			IF STYLE_PARTITIONWIZARD=1
			{
				...
				OLD_PARTITIONWIZARD_EXE_WINDOW_STR=%OLD_PARTITIONWIZARD_EXE_WINDOW_STR% -- %ID_STRING%
				; ---------------------------------------------------
				; 10 IS THE DEFAULT LENGTH FOR 1 BLOCK OF DATA
				; ---------------------------------------------------
				LENGTH_OLD := StrLen(OLD_PARTITIONWIZARD_EXE_WINDOW_STR) ; OF MULTI
				LEN_STRING:=StrLen(ID_STRING)                        ; OF ONE 
				LEN_STRING*=10     ; LENGTH OF OLD BIGGER COMPARE ONE SAMPLE -- MULTIPLE OF 1
				; ---------------------------------------------------
				; OF HERE ROUTINE ONLY EVER DO ONE SHOT
				...
				LOOP
				{
					StringTrimRIGHT, OLD_PARTITIONWIZARD_EXE_WINDOW_STR, OLD_PARTITIONWIZARD_EXE_WINDOW_STR, 10
					LENGTH_OLD := StrLen(OLD_PARTITIONWIZARD_EXE_WINDOW_STR)
					IF LENGTH_OLD>LEN_STRING
						BREAK
				}
			}
...
VBS_29_COPY_CAMERA_PHOTO_IMAGES_VBS:
	...
	IfWinEXIST VBS 29-COPY CAMERA PHOTO IMAGES.VBS ahk_class #32770
	{
		...
		VBS_29_COPY_CAMERA_PHOTO_IMAGE_SOUND_PING_COUNTER+=1
	...
	IfWinNOTEXIST VBS 29-COPY CAMERA PHOTO IMAGES.VBS ahk_class #32770
		VBS_29_COPY_CAMERA_PHOTO_IMAGE_SOUND_PING_COUNTER=0
...
EXIST_WSCRIPT_EXE_SOUND_PLAY_01:
	...
	; SOUNDPLAY, %A_SCRIPTDIR%\AUTOKEY -- 10-READ MOUSE CURSOR ICON\COMPLETE - AMPAR.WAV, 1 ; 1 IS WAIT
RETURN
EXIST_WSCRIPT_EXE_SOUND_PLAY_02:
	SETTIMER EXIST_WSCRIPT_EXE_SOUND_PLAY_02,OFF
	; SOUNDPLAY, %A_SCRIPTDIR%\AUTOKEY -- 10-READ MOUSE CURSOR ICON\COMPLETE - AMPAR.WAV, 1 ; 1 IS WAIT
RETURN
...
TIMER_COPY_SYNC_VBSCRIPT_CODE_SYNC_ER:
	...
	If NOT ErrorLevel
	{
		FN_VAR:="C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 32-COPIER_SYNC.VBS"
		IfExist, %FN_VAR%
		{
...
; ONLY AFTER TIME WHEN LOOP COME AROUND AGAIN
; -------------------------------------------------------------------
...
FN_VAR:="C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 23-MY IP.VBS"
IfExist, %FN_VAR%
	{
		Run, %FN_VAR%
	}
...
FN_VAR:="C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 24-I_VIEW32 CONVERT_CCSE.AHK"
IfExist, %FN_VAR%
	{
		Run, %FN_VAR%
	}
...
IF SET_GO=TRUE
{
	...
	VICE_VERSA_PATH=C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\# VV C DRIVE ROOT
...
; Run, "C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 13-COPY MOVE SHELL FOLDING.VBS" , , hide

Run, "C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 10-VICEVERSA _ SHELL FOLDERING__.AHK" /RUN

; Run, "C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 10-VICEVERSA _ SHELL FOLDERING__.AHK" , , hide
; C:\SCRIPTER\SCRIPTER CODE -- VB 02 VBSCRIPT\VBS 10-VICEVERSA _ SHELL FOLDERING__.VBS
