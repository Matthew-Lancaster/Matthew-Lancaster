...
Function InstanceToWnd(ByVal target_pid As Long, Target_exe$) As Long
'FindWindowPart
    ...
    'FindWindow2(ByVal 0&, ByVal 0&) Long's
'    test_hwnd = FindWindow2(ByVal 0&, ByVal 0&)
    test_hwnd = GetWindow(GetDesktopWindow(), GW_CHILD)
   ...
   '    End If
       ...
       If GetParent(test_hwnd) = 0 Then
         ...
         test_thread_id = GetWindowThreadProcessId(test_hwnd, _
                             test_pid)
         If test_pid = target_pid Or 1 = 1 Then
            'InstanceToWnd2 = test_hwnd
            ash$ = GetWindowTitle(test_hwnd)
'            If InStr(ash$, "Tidal Info Ver") Then
            If InStr(ash$, Target_exe$) Then
             InstanceToWnd = test_hwnd: Exit Do
            End If
            'Exit Do
         End If
       End If
...
Public Function FindWinPart2() As Long
...
Loop
FindWinPart2 = False
If Huge > 0 Then FindWinPart2 = Huge
