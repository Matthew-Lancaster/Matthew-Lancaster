...
Begin VB.Form Form1 
   BackColor       =   &H00800000&
   Caption         =   "ROBOFORM_LOADER"
   ClientHeight    =   5904
   ClientLeft      =   588
   ClientTop       =   5148
   ClientWidth     =   14052
   Icon            =   "ROBO_FORM_LOADER_FORM.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   5904
   ScaleWidth      =   14052
   Begin VB.Timer TIMER_ROBO_CURRENT_ACTIVE 
      Interval        =   1000
      Left            =   8928
      Top             =   3432
   End
   ...
   Begin VB.Timer TIMER_RETRY_WRITE_INFO_UNTIL_DONE1 
      ...
      Top             =   2292
   End
   Begin VB.Timer TIMER_WebCam_Matt_Timer 
      Interval        =   1000
      Left            =   4980
      Top             =   3348
   End
...
If IsIDE = True And KeyCode = 27 Then Unload Me
...
For Each Control In Form1.Controls
    I1 = UCase(Control.Name)
    If Mid(I1, 1, 5) = "TIMER" Then I1 = Mid(I1, 1, 5)
    If Mid(I1, 1, 4) = "FILE" Then I1 = Mid(I1, 1, 4)
    If Mid(I1, 1, 4) = "LIST" Then I1 = Mid(I1, 1, 4)
    
    NAME_EXCLUDE = "__TIMER MNU_ FILE LIST"
    
    If InStr(NAME_EXCLUDE, I1) > 0 Then
...
For Each Control In Form1.Controls
    ...
    XX2 = 0
    If InStr(I1, "ACTIVE") > 0 Then XX2 = 1
    If InStr(I1, "RESTRICT") > 0 Then XX2 = 1
    If InStr(I1, "RESTRICT") > 0 Then XX2 = 1
...
Label2 = "ROBOFORM C: DRIVE DESKTOP ___ LOADER __ MUST KILL ROBOFORM PORTABLE TO RUN"
Label3 = "ROBOFORM C: DRIVE PORTABLE __ LOADER __ RUN __ EASY LOADER"
Label4 = "ROBOFORM F: DRIVE PORTABLE __ LOADER __ RUN __ EASY LOADER"

Call Form_Resize_MANUAL_SUB

Call ROBO_CURRENT_LOADED

TIMER_ROBO_CURRENT_ACTIVE.Enabled = True

End Sub

Private Sub Form_Load_KAT_MP3_RELOAD()
...
Private Sub Form_Resize_MANUAL_SUB()
...
For Each Control In Form1.Controls
    ...
    On Error Resume Next
    IERR = 0
    IERR = Control.Visible
    On Error GoTo 0
    If Mid(UCase(Control.Name), 1, 3) = "MNU" Then IERR = 0
    
'    Debug.Print Control.Name
    'If Control.Name = "Label4" Then Stop
    If IERR = -1 Then
        XW1 = Control.Left + Control.Width
        If XW1 > X_MAX Then X_MAX = XW1
        
        YH1 = Control.Top + Control.Height
        If YH1 > Y_MAX Then Y_MAX = YH1
    End If
Next
...
'Menu_Height2 = 21
If Menu_Height > 0 Then MENU_OFFSET = 20
Me.Width = XW1 + 170 - 40
Me.Height = ((YH1 + (Menu_Height * Screen.TwipsPerPixelY)) + 1800 + MENU_OFFSET)
...
Dim ROBO_FORM_MSGBOX_CRASH_HWND As Long

'EXE_STRING = cProcesses.GetFileFromHwnd(HWND_O)

'PID = -1: HWND_O = 0
'Var = cProcesses.GetEXEID(PID, "C:\Program Files (x86)\Siber Systems\AI RoboForm\robotaskbaricon.exe")
'Var = cProcesses.Convert(PID, HWND_O, cnFromProcessID Or cnTohWnd)
'If HWND_O > 0 Then
'    RESULT = PostMessage(HWND_O, WM_CLOSE, 0&, 0&)
'    POSTMESSAGE_CLOSE_ALL_PARENT (HWND_O)
'End If
'
'PID = -1: HWND_O = 0
'Var = cProcesses.GetEXEID(PID, "C:\PortableRoboForm.exe")
'Var = cProcesses.Convert(PID, HWND_O, cnFromProcessID Or cnTohWnd)
'If HWND_O > 0 Then RESULT = PostMessage(HWND_O, WM_CLOSE, 0&, 0&)
'
'PID = -1: HWND_O = 0
'Var = cProcesses.GetEXEID(PID, "F:\PortableRoboForm.exe")
'Var = cProcesses.Convert(PID, HWND_O, cnFromProcessID Or cnTohWnd)
'If HWND_O > 0 Then RESULT = PostMessage(HWND_O, WM_CLOSE, 0&, 0&)
'
'Shell "TASKKILL /F /IM ROBO* /T", vbHide

'--------------------
'ONLY ONE ABLE FINDER
'--------------------
BEEP
ROBO_FORM_HWND = FindWinPart("RfAUTOSAVE")
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

ROBO_FORM_HWND = FindWinPart("RoboForm Taskbar Icon")
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

POSTMESSAGE_CLOSE_ALL_PARENT (ROBO_FORM_HWND)
...
If HWND_O > 0 Then ROBO_FORM_HWND = HWND_O
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

'Var = cProcesses.GetEXEID(PID, "portableRoboForm.exe")
PID = -1
Var = cProcesses.GetEXEID(PID, "robotaskbaricon.EXE")
Var = cProcesses.Convert(PID, HWND_O, cnFromProcessID Or cnTohWnd)
If HWND_O > 0 Then ROBO_FORM_HWND = HWND_O
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

'ROBO_FORM_HWND = FindWinPart_DEBUGGING("Robo")
ROBO_FORM_HWND = FindWinPart_KILL_ROBOFORM("RoboFORM ")
ROBO_FORM_HWND = FindWinPart_KILL_ROBOFORM("Robo")
ROBO_FORM_HWND = FindWinPart_KILL_ROBOFORM("RF")


ROBO_FORM_HWND = FindWinPart("RoboForm ToolBar")
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

ROBO_FORM_HWND = FindWinPart("RoboForm Batch Login")
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

ROBO_FORM_HWND = FindWinPart("RoboForm Search Box")
If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)

'PID = -1
'Var = cProcesses.GetEXEID(PID, "rf-chrome-nm-host.exe")
'Var = cProcesses.Convert(PID, HWND_O, cnFromProcessID Or cnTohWnd)
'If HWND_O > 0 Then ROBO_FORM_HWND = HWND_O
'If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)
'POSTMESSAGE_CLOSE_ALL_PARENT (ROBO_FORM_HWND)
...
'RoboForm ToolBar
'RoboForm Batch Login
'RoboForm Search Box
'RoboForm Taskbar Icon

'POSTMESSAGE_CLOSE_ALL_PARENT (ROBO_FORM_HWND)

'ROBO_FORM_HWND = FindWinPart("Robo")
'If ROBO_FORM_HWND > 0 Then RESULT = PostMessage(ROBO_FORM_HWND, WM_CLOSE, 0&, 0&)
'RESULT WORKING
'--------------------

'------------------------------------------------------------------------------------
'BY KILL ALL THE HWND SEEM RENDER IT UNABLE TO OPERATE AND THEN IS TO KILL FORCEFULLY
'CARE NOT TO DO WHILE SYCRONIZE
'------------------------------------------------------------------------------------

TIME_WAIT = Now + TimeSerial(0, 0, 4)
Do

    ROBO_FORM_HWND = FindWinPart("RoboForm Taskbar Icon")
    If ROBO_FORM_HWND > 0 Then Sleep 500
    DoEvents

Loop Until ROBO_FORM_HWND = 0 Or UNLOAD_FORM_FLAG = True Or TIME_WAIT < Now
...
Do
    ...
    If ROBO_FORM_HWND = 0 Then ROBO_FORM_HWND = FindWinPart("RoboForm ToolBar")
    If ROBO_FORM_HWND > 0 Then Sleep 500
    DoEvents

    If ROBO_FORM_HWND = 0 Then ROBO_FORM_HWND = FindWinPart("RoboForm Batch Login")
    If ROBO_FORM_HWND > 0 Then Sleep 500
    DoEvents

    If ROBO_FORM_HWND = 0 Then ROBO_FORM_HWND = FindWinPart("RoboForm Search Box")
    If ROBO_FORM_HWND > 0 Then Sleep 500
    DoEvents

Loop Until ROBO_FORM_HWND = 0 Or UNLOAD_FORM_FLAG = True
...
Sub ROBO_CURRENT_LOADED()

ROBO_FORM_HWND = FindWinPart("RoboForm Taskbar Icon")
If OROBO_FORM_HWND = ROBO_FORM_HWND Then
    Exit Sub
End If

OROBO_FORM_HWND = ROBO_FORM_HWND

EXE_STRING = cProcesses.GetFileFromHwnd(ROBO_FORM_HWND)

Label1.Caption = "QUICK SWAP __ CURRENTLY __ " + EXE_STRING
...
'------------------------------------------------------------------------------------------------------------------
'ROBO HAS A LOAD FROM MIAN C DRIVE IF REQUEST LOAD ANOTHER HANDLE AS AlignConstants SWAP AND OPTION UNLOAD PORTABLE
'IN THAT CASE HAVE TO UNLOAD WHATEVER ONE TO GET ONE WANTER
'------------------------------------------------------------------------------------------------------------------

'--------------------------------
'PORTABLE ROBOFORM KILL EASY
'MAIN ROBOFORM ONE AS A HERE HARD
'--------------------------------

Call KILL_ROBO_FORM

BEEP
Shell "C:\Program Files (x86)\Siber Systems\AI RoboForm\robotaskbaricon.exe"
...
'---------------------------------------------------------------------------
'PORTABLE ROBOFORM KILL EASY __ NOT QUITE IT LEAVE THE LOADER OF C DRIVE AND USE THE NLOAD PORTABLE OPTION
'HARDER KILL LIKE 1
'MAIN ROBOFORM ONE HARD
'---------------------------
'AT LEAST I CAN LOAD WHICH EVER ONE I WANT FOR NOW WITH MAIN ONE OUT THE WAY
'---------------------------------------------------------------------------
'FINISH AFTER DAY
'WEDNESDAY 08 MARCH
'---------------------------------------------------------------------------
...
'Shell "F:\PortableRoboForm.exe", vbNormalFocus
...
Private Sub MNU_BOOT_KILLER_Click()
BEEP
Me.WindowState = vbMinimized
...
Private Sub TIMER_ROBO_CURRENT_ACTIVE_Timer()
...
If IsIDE = True Then
    VB_EXE = FindWindow("ThunderRT6FormDC", "ROBOFORM LOADER")
...
Function KAT_MP3_IF_NOT_EXIST_AND_THEN_SHELL()
    ...
    KAT_MP3_IF_NOT_EXIST_AND_THEN_SHELL = True
...
Private Sub TIMER_WebCam_Matt_Timer_Timer()
...
Function FindWinPart_KILL_ROBOFORM(FIND_STRING) As Long
...
'FindWinPart_DEBUGGING = False
FIND_STRING = UCase(FIND_STRING)
Do While Test_HWND <> 0
    
    TEXT_TITLE = GetWindowTitle(Test_HWND)
    
    cText = Space$(255)
    CLASS_TEXT = GetClassName(Test_HWND, cText, 255)
    If TEXT_TITLE <> "" Then
         If InStr("-" + UCase(TEXT_TITLE), FIND_STRING) > 0 Then
            If IsIDE = True Then Debug.Print "-----------------------"
            If IsIDE = True Then Debug.Print "TITLE __ " + TEXT_TITLE
            XGO = 0
            If UCase(TEXT_TITLE) = UCase(Me.Caption) Then XGO = 1
            If Replace(UCase(TEXT_TITLE), " ", "_") = Replace(UCase(Me.Caption), " ", "_") Then XGO = 1
            If InStr(UCase(TEXT_TITLE), UCase(Me.Caption)) > 0 Then XGO = 1
            If InStr(Replace(UCase(TEXT_TITLE), " ", "_"), UCase(Me.Caption)) > 0 Then XGO = 1
            
            i = cText
            If IsIDE = True Then Debug.Print "CLASS __ " + cText
                            
            XGO2 = 1
            If Mid(cText, 1, 2) = "Rf" Then XGO2 = 0
            If Mid(cText, 1, 3) = "Sib" Then XGO2 = 0
                
                
            If XGO = XGO + 10 Or XGO2 = 0 Then
                If IsIDE = True Then Debug.Print "KILL GO TRUE"
                POSTMESSAGE_CLOSE_ALL_PARENT (Test_HWND)
                RESULT = PostMessage(Test_HWND, WM_CLOSE, 0&, 0&)
            Else
                If IsIDE = True Then Debug.Print "KILL GO FALSE"
            End If
            'FindWinPart_DEBUGGING = Test_HWND
            'Exit Do
        End If
    End If
...
Function FindWinPart_DEBUGGING(FIND_STRING) As Long
...
Do While Test_HWND <> 0
    ...
    If TEXT_TITLE <> "" Then
         If InStr("-" + UCase(TEXT_TITLE), FIND_STRING) > 0 Then
            If IsIDE = True Then Debug.Print TEXT_TITLE
            FindWinPart_DEBUGGING = Test_HWND
            'Exit Do
        End If
    End If
...
Do While Test_HWND <> 0
    ...
    If ASH$ <> "" Then
         If InStr("-" + UCase(ASH$), FIND_STRING) > 0 Then
            If IsIDE = True Then Debug.Print ASH$
