...
Begin VB.Form Form1 
   ...
   Begin VB.Timer TIMER_SET_FORM_START_POSITION_01 
      ...
      Top             =   2556
   End
   Begin VB.Timer TIMER_FORM_LOAD_INIT 
      Enabled         =   0   'False
      Interval        =   1
      Left            =   13164
      Top             =   1956
   End
   Begin VB.Timer TIMER_REBOOT_ROUTINE 
      Enabled         =   0   'False
      Interval        =   100
      Left            =   12804
      Top             =   2124
   ...
   Begin MSComctlLib.ListView ListView_VB_MODIFIED_ERROR 
      ...
      NumItems        =   0
   End
   Begin MSComctlLib.ListView lstProcess_4_SORT_PID 
      Height          =   576
      Left            =   9624
      TabIndex        =   163
      Top             =   6312
      ...
      NumItems        =   0
   End
   Begin MSComctlLib.ListView lstProcess_8_NATURAL 
      Height          =   576
      Left            =   11508
      TabIndex        =   164
      Top             =   6324
   ...
   Begin VB.Label Label_ALPHA_A_Z 
      ...
      Width           =   1392
   End
   Begin VB.Label Label_BUTTON_ON_TOP 
      AutoSize        =   -1  'True
      BackColor       =   &H00DFFFFF&
      Caption         =   "ON TOP"
      BeginProperty Font 
   ...
   Begin VB.Label Label_FILE_LOCATOR_GS_TEMP 
      ...
      Width           =   1896
   End
   Begin VB.Label Label_TRIPPER_KILL_ALL_RELOAD_AHK 
      Alignment       =   2  'Center
      AutoSize        =   -1  'True
      BackColor       =   &H00DBFFDE&
      Caption         =   "TRIPPER"
   ...
   Begin VB.Menu MNU_TEAMVIEWER 
      Caption         =   "TEAMVIEWER"
   End
   Begin VB.Menu MNU_TIMEZONE_CLOCK_02 
      Caption         =   "TIMEZONE_CLOCK"
   End
   Begin VB.Menu MNU_COMPUTER 
      Caption         =   "COMPUTER --"
   End
   Begin VB.Menu MNU_MY_COMPUTER_FOLDER 
      Caption         =   "MY COMPUTER FOLDER"
   End
   Begin VB.Menu MNU_RECYCLEBINFOLDER 
      Caption         =   "RECYCLE BIN FOLDER"
   End
   Begin VB.Menu MNU_DOWNLOAD_FOLDER 
      Caption         =   "DOWNLOAD FOLDER"
   End
   Begin VB.Menu MNU_SCRIPTOR_FOLDER 
      Caption         =   "SCRIPTOR FOLDER"
   End
   Begin VB.Menu MNU_EXIT_02 
      Caption         =   "EXIT"
   End
End
...
' --------------------------------------------------------------
' NOTE -- HERE ROUTINE TAKE OFF -- NOT WORK FOR WANTER
' STOP FEW PROG HARSH WHEN AHK STOP
' Tue 06-Oct-2020 19:08:00
' NOT HAD CHANGE -- REMOVE SOME ADD OTHER
' --------------------------------------------------------------
...
Dim VAR_DATE_ORIGINAL_APP_EXE_WHEN_PROGRAM_START


Dim OLD_NOT_RESIZE_EVENTER

Dim A1, A2, B1
Dim HUGE, VAR_STRING

Dim OLD_HOUR_NOW_TO_RUN_AUTOHOTKEY
Dim TIME_TO_RUNNER_CHECK_AUTOHOTKEY_IS_RUNNER

Dim TOP_AND_LEFT_ONCE_ONLY
...
Dim CURRENT_LSTPROCESS_LISTCOUNT
Dim LSTPROCESS_3_SORTER_LISTVIEW_MOUSEDOWN_BUTTON
Dim TOP_INDEXER As Integer
Dim MOUSE_BUTTON_FOR_LABEL_ALPHA_A_Z_MOUSEDOWN
...
' ------------------------------------------------------------------
' HERE USE FOR VB6 THE CODE IS REPEATER WITH VBS VBSCRIPT METHOD
' ------------------------------------------------------------------
' -------------------------------------------------------------------
...
' --------------------------------------------------------------
' SESSION 012 -- WORK
' ADD Function FIND_SCRIPTNAME(PID_TEST)
' --------------------------------------------------------------
' FIRST THING OF ALL
...
'
' ALL DAY
' MADE THE LISTVIEW MORE
' HAS INDICATE BOTH SIDE WHAT PID NUMBER
' ENSURE VISIBLE
'
' NEW CLIPBOARD ROUTINE
'
' AND NEW TEXT BOX
' SHOW WHAT COMMAND LINE CALL PROGRAM AND GIVE WSCRIPT AND MORE
' WHAT THEY SCRIPT NAME ARE
'
' MAKE TRIPPER MISSING PULSE DETECTOR
' AND SHUT DOWN ITEM WHEN HOLD THING
'
' --------------------------------------------------------------
' FROM -- Thu 23-Jan-2020 11:01:12
...
' SESSION 015 -- WORKER
' --------------------------------------------------------------
' MAKE MSGBOX WITH POPUP VERSION AND IT REMOVER AFTER TIMEROUT
' WITHOUT MODIFY BUTTON
' --------------------------------------------------------------
' Tue 09-Jun-2020 09:17:25
' Tue 09-Jun-2020 10:02:24 -- 44 MINUTE
' --------------------------------------------------------------

' --------------------------------------------------------------
' SESSION SIXTHTEENTHAR 016 -- WORKER
' --------------------------------------------------------------
' ADD RECYLE BIN LOAD IF ONLY HAS SOME IN AT MIDNIGHT
' TRY VB2008 VERSION FIRST AND NOT WORKER
' AND EASY VB6 -- 1 HOUR 8 MINUTE
' --------------------------------------------------------------
' Tue 09-Jun-2020 10:11:44
' Tue 09-Jun-2020 11:20:00 -- 1 HOUR 8 MINUTE
' --------------------------------------------------------------



' STRESS VARIABLE KILL SOME GONE
' X_STRESS_LEVEL
Dim UPDATE_O_NOW_LV_AUTOSIZE_1

Dim LSTPROCESS_2_LISTVIEW_COLUMNCLICK_VAR

Dim TREESIZE_EXE

Dim Label_ALPHA_A_Z_BEEN_SET_BEFORE

Dim GET_COLOR_SET_DONE_BEFORE

Dim ARRAY_CB_2()

Dim COMMAND_LINE

Dim OLD_Timer_Pause_Update_Enabled
...
Dim LSTPROCESS_2_ENSUREVISIBLE_VALUE
Dim LSTPROCESS_3_ENSUREVISIBLE_INDEX
Dim LSTPROCESS_3_ENSUREVISIBLE_VALUE

Dim OLD_PROCESS_PID_STORE_LST_02
Dim OLD_PROCESS_PID_STORE_LST_03

Dim OLD_HWND_CABINETWCLASS_E_HDD_HARDWARE
...
'
'Declare Function GetScrollInfo Lib "user32" (ByVal hWnd As IntPtr, ByVal n As Integer, ByRef lpScrollInfo As SCROLLINFO) As Integer
...
Private Declare Function ScrollWindowEx Lib "user32" (ByVal hWnd As Long, ByVal dx As Long, ByVal dy As Long, lprcScroll As RECT, lprcClip As RECT, ByVal hrgnUpdate As Long, lprcUpdate As RECT, ByVal fuScroll As Long) As Long
...
' ----------------------------------------------------------------------------
' Me.Show
' RESULT = MessageBoxTimeout(0&, I_1, App.EXEName, MB_SETFOREGROUND Or IDOKONLY Or MB_ICONASTERISK, 0&, 10000&)
' RESULT = MessageBoxTimeout(Me.hWnd, I_1, App.EXEName, MB_SETFOREGROUND Or IDOKONLY Or MB_ICONASTERISK, 0&, 10000&)
...
'If GetComputerName = "8-MSI-GP62M-7RD" Then End


'    -----------------------------------------------
'    THE MAIN TIMER SET AT STARTER
'    -----------------------------------------------
'    Timer_FOREGROUND_WINDOW_CHANGE.Enabled = False
'    Timer_1_SECOND.Enabled = False
'    Timer_MOUSE_CORD.Enabled = False
'    ONE_MILLISECOND_2_Timer.Enabled = False
'    Timer_FOREGROUND_WINDOW_CHANGE_02.Enabled = False
'    Timer_GET_KEY_ASYNC_STATE.Enabled = False
'    Timer_VB_MAXIMIZE.Enabled = False
...
' ------------------------------------------------------------------------------
    ...
    ListView_VB_MODIFIED_ERROR.Enabled = False

    MNU_NOT_MINIMIZE_VALUE = False
    Call MNU_NOT_MINIMIZE_SET
    
    ' ----------------------
    ' WHAT TIMER ARE ENABLER
    ' ----------------------
    ReDim ARRAY_TIMER_CONTROL(100)
'    Dim CONTROL As CONTROL
'    Dim I
    i = 0
    ...
    ' GET A 1ST TIMER
    OLD_HOUR_NOW_TO_RUN_AUTOHOTKEY = Hour(Now)
    TIME_TO_RUNNER_CHECK_AUTOHOTKEY_IS_RUNNER = Now - TimeSerial(0, 2, 0)
    
    Label_KILL_AUTOHOTKEY.BackColor = RGB(255, 200, 200)
    Label_KILL_WSCRIPT.BackColor = RGB(255, 200, 200)
    Label24.BackColor = RGB(255, 200, 200)
    
    
    
    Do
    ' SOMEHOW FORM_NOTDISPLAY PROPER WHEN HERE A WORK-AROUND WHNE CROSS MIDNIGHT
    Loop Until DateDiff("s", Int(Now), Now) > 90
...
Dim PID_VALUE As Long
PID_VALUE = TxtPID
i = Mid(lstProcess.List(R_I), 1, 7 - 1)

Thread_Resume (ITEM_ADD_10)
...
Thread_Suspend (PID_VALUE)

End Sub

Private Sub MNU_DOWNLOAD_FOLDER_Click()
    Me.WindowState = vbMinimized
...
Private Sub MNU_EXIT_02_Click()
End
End Sub

Private Sub MNU_MY_COMPUTER_FOLDER_Click()
    Me.WindowState = vbMinimized
    Shell "EXPLORER shell:MyComputerFolder", vbMaximizedFocus
End Sub

Private Sub MNU_RECYCLEBINFOLDER_Click()
    Me.WindowState = vbMinimized
...
Sub TIMER_FORM_LOAD_INIT_TIMER()
    TIMER_FORM_LOAD_INIT.Enabled = False
    Call FORM_LOAD_STEP_02
End Sub

Sub FORM_LOAD_STEP_02()
...
'    Debug.Print XX
    ...
    End If
    ' --------------------------------------------------------
    ' HERE REQUIRE DELETE AT START CODE
    ' OR CODE SENSITIVE TO RUN THING
    ' --------------------------------------------------------
    ...
    With LSTPROCESS_2_LISTVIEW
        .ColumnHeaders.Add , "PID", "PID", 700 - 50, lvwColumnLeft
        .ColumnHeaders.Add , "", "", COLUMN_02_WIDTH, lvwColumnLeft
        .ColumnHeaders.Add , "EXE", "EXE - PID NATURAL SORTER", 9000, lvwColumnLeft
        .View = lvwReport
        .height = 7000
        .FullRowSelect = True
    End With
    ...
    If GetComputerName = "4-ASUS-GL522VW" Then
        With ListView_CPU_INFO
            .ColumnHeaders.Add , "CPU", "CPU", 2200, lvwColumnLeft
            .ColumnHeaders.Add , "INFO", "INFO", 3500, lvwColumnLeft
            .View = lvwReport
            .FullRowSelect = True
        End With
    Else
...
'    Me.WindowState = vbNormal
    ...
    ' COMMAND_LINE = "MINIMAL"
    If InStr(COMMAND_LINE, "MINIMAL") > 0 Then OR_LOGIC = 2
    If InStr(COMMAND_LINE, "MIN") > 0 Then OR_LOGIC = 2
    If InStr(COMMAND_LINE, "MAXIMUM") > 0 Then OR_LOGIC = 1
    If InStr(COMMAND_LINE, "TASKBAR") > 0 Then OR_LOGIC = 1
    If InStr(COMMAND_LINE, "T") > 0 Then OR_LOGIC = 1        ' WHEN CALL BY TASKBAR ALLOW OPERATE VBNORMAL
    ...
    WIN_STATE_VAR = 0
    If InStr(COMMAND_LINE, "T") > 0 Then WIN_STATE_VAR = 1
    ...
    If RUN_FROM_OWN_PROJECT_UPDATE = False Then
        If COMMAND_LINE = "R" Then Me.WindowState = vbMinimized
    End If
    ...
    I_1 = Trim(Str(20)) + " __ COUNT __" + vbCrLf
    ...
    ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH_COUNT = 0
    TIMER_SET_FORM_START_POSITION_01.Enabled = True
    
    Call CMDMOVENORMAL_STATER
...
' SOME RESIZE WORK FORM POSITION DO BY HERE TIMER
' TIMER_SET_FORM_START_POSITION_01_TIMER()

If picCrossHair_MouseMove_Dragging_VAR = True Then Exit Sub


If DID_FORM_LOAD = True Then
If FIRST_TIME_WINDOWSTATEUP_FROM_VBMINIMIZED = False Then
If Me.WindowState = vbNormal Then
    ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH_COUNT = 0
    NOT_RESIZE_EVENTER = False
    TOP_AND_LEFT_ONCE_ONLY = False
    DID_FORM_LOAD = False
End If
End If
End If

' Debug.Print Now & " -- " & Me.height
...
If FIRST_TIME_WINDOWSTATEUP_FROM_VBMINIMIZED = False Then
    If Me.WindowState <> vbMinimized Then
        FIRST_TIME_WINDOWSTATEUP_FROM_VBMINIMIZED = True
    End If
    O_Me_WindowState = 0
    RESIZE_LOOP_STOP = False
    TOP_AND_LEFT_ONCE_ONLY = False
    Call TIMER_SET_FORM_START_POSITION_01_TIMER
...
Sub TIMER_SET_FORM_START_POSITION_01_TIMER()
    
    Dim MATRIX_VAR
    Dim MATRIX_VAR_1, MATRIX_VAR_2
    ...
    ' ------------------------------------------------
    TT = Me.Top + Me.Left + Me.height + Me.width
    If TT = ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH Then
        i = ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH_COUNT
        i = i + 1
        ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH_COUNT = i
    End If
    ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH = TT
    ' ------------------------------------------------
    ' WHEN ALL POSITION SAME TEN TIME STOP TIMER
    ' ------------------------------------------------
    If ME_TOP__ME_LEFT__ME_HEIGHT__ME_WIDTH_COUNT > 30 Then
        TIMER_SET_FORM_START_POSITION_01.Enabled = False
    End If
    ' ------------------------------------------------
    
    ' ------------------------------------------------
    If TOP_AND_LEFT_ONCE_ONLY = False Then
        Me.Top = 0 '20 'TOP_HEIGHT 'Screen.Height / 2 - Me.Height / 2 - 800
        MATRIX_VAR_1 = GetSystemMetrics(SM_CXSCREEN) * Screen.TwipsPerPixelX / 2 - Me.width / 2
        ' MATRIX_VAR_2 = * Screen.TwipsPerPixelX / 2 - Me.width / 2
        
        'MATRIX_VAR_2 = Screen.height / 2 - Me.height / 2
        MATRIX_VAR_2 = Screen.width / 2 - Me.width / 2
        MATRIX_VAR = MATRIX_VAR_1
        If MATRIX_VAR_2 > MATRIX_VAR_1 Then MATRIX_VAR = MATRIX_VAR_2
        If MATRIX_VAR_1 > MATRIX_VAR_2 Then MATRIX_VAR = MATRIX_VAR_1
        
        If MATRIX_VAR > 0 Then
            Me.Left = MATRIX_VAR
        
            If Me.Top = 0 And Me.Left = MATRIX_VAR Then
                TOP_AND_LEFT_ONCE_ONLY = True
            End If
        End If
    End If
    ' ------------------------------------------------
...
Sub SET_HEIGHT()
    ...
    Next
    ' LINE 0005
    Me.height = ME_HEIGHT + (Menu_Height * Screen.TwipsPerPixelY) + 650
    ' SET HERE WHERE TOP VALUE TO BE DONE END VALUE HIGHER MAKE FORM HEIGHT MORE
    ' --------------------------------------------------------------------------
    On Error GoTo 0
End Sub

Sub TIMER_SET_FORM_START_POSITION_02()
...
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_1X"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_2E"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_3L"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_4G"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_5P"
LDAC = LDAC + 1
...
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_TRIPPER_KILL_ALL_RELOAD_AHK"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_FILE_LOCATOR_GS_TEMP"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_BUTTON_ON_TOP"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_1X"
LDAC = LDAC + 1
ARRAY_CB(LDAC) = "Label_CN_2E"
...
Dim VAR_DATE_1
Dim VAR_DATE_4
Dim FILE_NAME, SCRIPT_VBS_NAME, X1_FILENAME
Dim F1, F2

FILE_NAME = App.Path + "\" + App.EXEName + ".EXE"
Set F1 = FSO.GetFile(FILE_NAME)
VAR_DATE_1 = 0
VAR_DATE_1 = F1.DateLastModified

X1_FILENAME = Replace(FILE_NAME, "\VB6\", "\VB6-EXE\")
Set F2 = FSO.GetFile(X1_FILENAME)
VAR_DATE_4 = 0
VAR_DATE_4 = F1.DateLastModified

If VAR_DATE_4 > VAR_DATE_1 Then
    Reset
    If FileInUse(FILE_NAME) = True Then Exit Sub
    If FileInUse(X1_FILENAME) = True Then Exit Sub
    FSO.CopyFile X1_FILENAME, FILE_NAME
End If

If VAR_DATE_ORIGINAL_APP_EXE_WHEN_PROGRAM_START < VAR_DATE_1 Then
    Reset
    If FileInUse(FILE_NAME) = True Then Exit Sub
...
For r = 1 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count
    ...
    End If
Next
' PUT OUR OWN EXE IN JUST IN CASE NOT RUNNER WHILE IDE ENVIROMENT
' AND A FEW MORE
...
' -------------------------------------------
LSTPROCESS_2_LISTVIEW.Font.Bold = True
LSTPROCESS_2_LISTVIEW.Font.Name = "Arial Rounded MT Bold"
LSTPROCESS_3_SORTER_LISTVIEW.Font.Bold = True
LSTPROCESS_3_SORTER_LISTVIEW.Font.Name = "Arial Rounded MT Bold" '"Source Code Pro Black"
' -------------------------------------------
...
' TOGGLE NOT WHEN ABOVE TWO SET
If SETTING_WIDTH_LISTVIEW = False Then
    SETTING_WIDTH_LISTVIEW = True
    ' LSTPROCESS_2_LISTVIEW.Font.Size = Default = 8.4
    LSTPROCESS_2_LISTVIEW.Font.size = 10
    LSTPROCESS_3_SORTER_LISTVIEW.Font.size = LSTPROCESS_2_LISTVIEW.Font.size
    'LSTPROCESS_2_LISTVIEW_HERE
    LSTPROCESS_2_LISTVIEW.width = 3850
End If
...
If GetComputerName = "2-ASUS-EEE" Then
    OFFSET_HEIGHT_SMALLER = 980 ' WORK NEGATIVE HIGHER IS SMALL
    LSTPROCESS_2_LISTVIEW.height = HY - LSTPROCESS_2_LISTVIEW.Top + 80 - OFFSET_HEIGHT_SMALLER
...
'Label53_Here
Label53.width = lstProcess_2_AND_lstProcess_3_WIDTH_POSTION_WITH_ER_OFFSET_LEFT - 30
...
' ListView_CPU_INFO.Left = 20 ' DEFAULT HERE ALREADY SET SOMEWHERE BEFORE
ListView_CPU_INFO.height = 1480
...
' If COMMAND_LINE <> "" Then Me.WindowState = vbMinimized
...
Private Sub Label_ALPHA_A_Z_Click(Index As Integer)

' Dim TOP_INDEXER As Integer

Dim FONT_SIZE_VALUE_01
Dim FONT_SIZE_VALUE_02

TOP_INDEXER = Index

If MOUSE_BUTTON_FOR_LABEL_ALPHA_A_Z_MOUSEDOWN = 2 Then Exit Sub

If Label_ALPHA_A_Z_BEEN_SET_BEFORE = False Then
...
'    If TOP_INDEXER = 0 Then
        ...
        FONT_SIZE_VALUE_01 = 33 '32 ' THE @ SIGN
        FONT_SIZE_VALUE_02 = 18 '18 ' ALL THE ALPHA
        
        ' FOR THE ALPHA SIDE SELECTION
        ' ------------------------------------
        If GetComputerName = "2-ASUS-EEE" Then
            FONT_SIZE_VALUE_01 = 29
        ...
        Dim ITEM_HEIGHT
        Dim ITEM_HEIGHT_STEP
        Dim LEFT_SIDE
        Dim ITEM_HEIGHT_STEP_STORE
        Dim LEFT_STEP
        Dim A_COUNT
        
        ' LINE 0003
        LEFT_SIDE = LSTPROCESS_3_SORTER_LISTVIEW.Left + LSTPROCESS_3_SORTER_LISTVIEW.width + 30
        
        Label_ALPHA_A_Z(0).AutoSize = True
        Label_ALPHA_A_Z(0).FontSize = FONT_SIZE_VALUE_01
        Label_ALPHA_A_Z(0).Refresh
        Label_ALPHA_A_Z(0).Left = LEFT_SIDE
        ' Label_ALPHA_A_Z(0).Top = lstProcess_3_SORTER_ListView.Top - Label_ALPHA_A_Z(0).height - 28
        Label_ALPHA_A_Z(0).Top = 0 'lstProcess_3_SORTER_ListView.Top - Label_ALPHA_A_Z(0).height - 28
        ' Label_ALPHA_A_Z(0).Top = Label_ALPHA_A_Z(1).Top - Label_ALPHA_A_Z(1).height
        Label_ALPHA_A_Z(0).AutoSize = False
        ' THE WIDTH AND HEIGHT OF SIGN OFFSET
        Label_ALPHA_A_Z(0).width = Label_ALPHA_A_Z(0).width + 80
        Label_ALPHA_A_Z(0).height = Label_ALPHA_A_Z(0).height + 80
        
        Dim R2
        ' THE HEIGHT OF ALPHA AFTER AUTOSIZE
        For R2 = 1 To Label_ALPHA_A_Z.Count - 1
            Label_ALPHA_A_Z(R2).height = Label_ALPHA_A_Z(R2).height + 40
        Next
        
        Dim ITEM_HEIGHT_GAP
        ' THE GAP
        ITEM_HEIGHT = Label_ALPHA_A_Z(1).height + 10
        ' THE GAP OF ALPHA
        ITEM_HEIGHT_GAP = 10
        ' Label_ALPHA_A_Z(1).height = Label_ALPHA_A_Z(1).height * 2 + 10
        ' Label_ALPHA_A_Z(1).height = Label_ALPHA_A_Z(1).height * 4 + 10
        
        ' 0001 OPTION -- THE HEIGHT OF ALPHA AFTER SIGN BOX
        ' 0002 OPTION -- HEIGHT FROM TOP LISTVIEW
        ITEM_HEIGHT_STEP = LSTPROCESS_3_SORTER_LISTVIEW.Top ' + 10
        ITEM_HEIGHT_STEP = Label_ALPHA_A_Z(0).height + Label_ALPHA_A_Z(0).Top + 50
        ITEM_HEIGHT_STEP_STORE = ITEM_HEIGHT_STEP ' + 20 ' + ITEM_HEIGHT
        LEFT_SIDE = LSTPROCESS_3_SORTER_LISTVIEW.Left + LSTPROCESS_3_SORTER_LISTVIEW.width + 30
        ...
        For Each Control In Label_ALPHA_A_Z
            If Control.Index <> 0 Then
                ...
                ' THE STEP GET HIGHER ---------------------------
                ' STEP RESET TO TOP HALF WAY --------------------
                ' IDEA DOUBLE AND FONT HIGHER
                ' -----------------------------------------------
            End If
        Next
End If
...
CHAR_A_Z_STR = UCase(Chr(TOP_INDEXER + 64)) ' ALREADY AS HERE REGUARD UCASE
If InStr(A_TO_Z_PROCESS_NAME, CHAR_A_Z_STR) = 0 Then
    For R_I = TOP_INDEXER To Label_ALPHA_A_Z.UBound
        TOP_INDEXER = TOP_INDEXER + 1
        If TOP_INDEXER < 27 Then
        CHAR_A_Z_STR = UCase(Chr(TOP_INDEXER + 64))
        If InStr(A_TO_Z_PROCESS_NAME, CHAR_A_Z_STR) > 0 Then
            ' CALL ROUTINE HERE AGAIN NEXT SUB STYLE
            ' AND GIVE A CHAR LETTER THAT FINDER
            ' OR REDO SOME CODE WHNT INSTRUCTION PASS VIRTUALLY THE SAME OR REWRITTER
            ' -----------------------------------------------------------------------
            If Index > 0 Then
                Call Label_ALPHA_A_Z_Click(TOP_INDEXER)
            End If
            Exit Sub
        End If
        End If
...
Call SET_HEIGHT
Call SET_POSITION_LISTVIEW3_AND_LISTVIEW2
    
End Sub

Private Sub SET_POSITION_LISTVIEW3_AND_LISTVIEW2()

Dim R_I
Dim A_TO_Z_PROCESS_NAME
Dim CHAR_A_Z_STR
If TOP_INDEXER = 0 Then Exit Sub

PROCESS_PID_STORE_LST_03 = -1
If TOP_INDEXER = 1 Then
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(1).EnsureVisible
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(1).Selected = True
End If
...
Dim FURTHER_ONE_VISIBLE_AND_THEN_PULL_UP
Dim STOP_NEGATIVE_VALUE
CHAR_A_Z_STR = UCase(Chr(TOP_INDEXER + 64))  ' ALREADY AS HERE REGUARD UCASE
If TOP_INDEXER > 0 Then
    For R_I = 1 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count
        If UCase(Mid(LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I).SubItems(2), 1, 1)) = CHAR_A_Z_STR Then
            FURTHER_ONE_VISIBLE_AND_THEN_PULL_UP = LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count
            LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(FURTHER_ONE_VISIBLE_AND_THEN_PULL_UP).EnsureVisible
            STOP_NEGATIVE_VALUE = R_I - 2
            If STOP_NEGATIVE_VALUE < 1 Then STOP_NEGATIVE_VALUE = 1
            LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(STOP_NEGATIVE_VALUE).EnsureVisible
            ...
            PROCESS_PID_STORE_LST_02 = LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I)
            Exit For
        End If
    Next
End If
...
Dim TINDEX
    ...
    For R_I = 1 To LSTPROCESS_2_LISTVIEW.ListItems.Count ' - 1
        If LSTPROCESS_2_LISTVIEW.ListItems(R_I) = PROCESS_PID_STORE_LST_02 Then
            ...
            ' ---- 4 FROM THE TOP AND ---- 2 FROM THE TOP
            SET_DOWN = R_I - 2
            ...
            If LSTPROCESS_2_ENSUREVISIBLE_INDEX > 0 Then
            If LSTPROCESS_2_ENSUREVISIBLE_INDEX <> R_I Then
            If LSTPROCESS_2_LISTVIEW.ListItems(R_I) <> LSTPROCESS_2_ENSUREVISIBLE_VALUE Then
                TINDEX = LSTPROCESS_2_ENSUREVISIBLE_INDEX
                ' INDICATE ITEM
...
Private Sub Label_ALPHA_A_Z_MouseDown(Index As Integer, Button As Integer, Shift As Integer, x As Single, y As Single)
    
    Exit Sub
    
End Sub

Private Sub Label_FILE_LOCATOR_GS_TEMP_Click()

Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
Label_FILE_LOCATOR_GS_TEMP.BackColor = RGB(255, 255, 255)
...
FN_VAR_1 = "C:\0 00 LINK SET QUICKER MOVER\SAVEDCRITERIA__FILE LOCATOR__00_GOODSYNC_ALL_LOCAL_DRIVE_GSDATA_-_SAVED_-_HISTORY_.srf"
FN_VAR_1 = "C:\0 00 LINK SET QUICKER MOVER\SAVEDCRITERIA__FILE LOCATOR__00_GOODSYNC_ALL_LOCAL_DRIVE_GSDATA_-_SAVED_-_HISTORY_.flwx"
Dim WSHShell
Set WSHShell = CreateObject("WScript.Shell")
    WSHShell.RUN """" + FN_VAR_1 + """", ShowWindow_2, DontWaitUntilFinished
Set WSHShell = Nothing
...
Public Sub Label_EXE_FILENAME_Click()
    On Error Resume Next
    Clipboard.Clear
    Clipboard.SetText Mid(TxtEXE.Text, InStrRev(TxtEXE.Text, "\") + 1)
    If Err.Number > 0 Then
        VAR_TIMER_CLIPBOARD_TIMER_RETRY = "Label_EXE_FILENAME_Click"
...
Private Sub cmdMoveNORMAL_Click()
...
Private Sub CMDMOVENORMAL_STATER()
    ...
    ' If txthWnd.Text <> "" Then
    i_Result = GetWindowRect(Me.hWnd, Rect_Get)
...
'    HY = 10
    ...
    MoveWindow Me.hWnd, HX, HY, HW, HH, True
    ' LHWND_Function_Button_Set_MIN_MAX = Val(Me.hWnd)
    ' Call ChunkCodeOnMouse

    If MNU_NOT_MINIMIZE_VALUE = False Then
        Me.WindowState = vbMinimized
        FIRST_TIME_WINDOWSTATEUP_FROM_VBMINIMIZED = False
    End If
...
Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
Label_TRIPPER_KILL_ALL_RELOAD_AHK.BackColor = RGB(255, 255, 255)
...
Private Sub Label_BUTTON_ON_TOP_Click()
Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
Label_BUTTON_ON_TOP.BackColor = RGB(255, 255, 255)
...
Private Sub LSTPROCESS_2_LISTVIEW_ColumnClick(ByVal ColumnHeader As MSComctlLib.ColumnHeader)

LSTPROCESS_2_LISTVIEW_COLUMNCLICK_VAR = Not LSTPROCESS_2_LISTVIEW_COLUMNCLICK_VAR

Call POPULATE_THE_LIST_VIEW

End Sub

Private Sub MNU_NOT_MINIMIZE_Click()
    MNU_NOT_MINIMIZE_VALUE = Not MNU_NOT_MINIMIZE_VALUE
    If MNU_NOT_MINIMIZE_VALUE = True Then
        MNU_NOT_MINIMIZE.Caption = "[__ NOT MINIMIZE OPERATION __ TRUE __]"
    Else
        MNU_NOT_MINIMIZE.Caption = "[__ NOT MINIMIZE OPERATION __ FALSE __]"
    End If
End Sub

Private Sub MNU_NOT_MINIMIZE_SET()
    If MNU_NOT_MINIMIZE_VALUE = True Then
        MNU_NOT_MINIMIZE.Caption = "[__ NOT MINIMIZE OPERATION __ TRUE __]"
    Else
        MNU_NOT_MINIMIZE.Caption = "[__ NOT MINIMIZE OPERATION __ FALSE __]"
...
Public Sub TEXT_EXE_FILENAME_Click()
    On Error Resume Next
    Clipboard.Clear
    Clipboard.SetText TEXT_EXE_FILENAME.Text
    If Err.Number > 0 Then
        VAR_TIMER_CLIPBOARD_TIMER_RETRY = "TEXT_EXE_FILENAME_Click"
        TIMER_CLIPBOARD_TIMER_RETRY.Enabled = True
        Exit Sub
    End If
...
Private Sub TEXT_EXE_FILENAME_Change()
' TEXT_EXE_FILENAME
End Sub

Private Sub Text_OS_INSTALL_DATE_02_Change()
'Text_OS_INSTALL_DATE_02
End Sub

Private Sub Timer_clear_imediate_Timer()
Timer_clear_imediate.Enabled = False
CLEAR_DEBUG_AREA.ClearInmediateWindow
End Sub
...
Public Sub EXPLORER_DISPLAY_MESSENGER()


    ' Dim HUGE, VAR_STRING
    Dim I_1 As String, QUITE_MSGBOX
    ...
    If HUGE > 0 Then
        On Error Resume Next
        Clipboard.Clear
        Clipboard.SetText I_1  'VAR_STRING
        If Err.Number > 0 Then
            VAR_TIMER_CLIPBOARD_TIMER_RETRY = "EXPLORER_DISPLAY_MESSENGER"
            TIMER_CLIPBOARD_TIMER_RETRY.Enabled = True
            Exit Sub
        End If
        On Error GoTo 0
...
Private Sub Label_CN_1X_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_1X.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_1X.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_2E_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_2E.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_2E.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_3L_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If

        VAR_LAB_TEXT = Label_CN_3L.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_3L.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_4G_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_4G.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_4G.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_5P_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_5P.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_5P.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_7G_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_7G.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_7G.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CN_8M_Click()
        If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
        
        VAR_LAB_TEXT = Label_CN_8M.Caption
        
        Call COLOUR_BOX_SELECTOR_RESTORE_DEFAULT
        Label_CN_8M.BackColor = RGB(255, 255, 255)
...
Private Sub Label_CHROME_PAGE_AUTO_ON_Click()
        ...
        Dim objShell
        Set objShell = CreateObject("Wscript.Shell")
        objShell.RUN """C:\SCRIPTER\SCRIPTER CODE -- AUTOHOTKEY\Autokey -- 79-BROWSER LOAD URL BOOT CHROME.ahk""", DontShowWindow, DontWaitUntilFinished
        Set objShell = Nothing
...
WINDOW_hWnd(1) = FindWindow(vbNullString, "EliteSpy+ 2001 __ www.PlanetSourceCode.com")
...
PROCESS_TO_KILLER = LSTPROCESS_3_SORTER_LISTVIEW.ListItems(LSTPROCESS_3_SORTER_LISTVIEW.SelectedItem.Index).SubItems(2)
...
Label29_Click
PROCESS_TO_KILLER_TO_GO = Label22.Caption
PROCESS_TO_KILLER_TO_GO = Replace(PROCESS_TO_KILLER_TO_GO, "TASKKILL ", "")
Label23_Click
...
Private Sub lstProcess_3_SORTER_ListView_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    LSTPROCESS_3_SORTER_LISTVIEW_MOUSEDOWN_BUTTON = Button
...
Private Sub LSTPROCESS_2_LISTVIEW_DblClick()
    PROCESS_TO_KILLER = LSTPROCESS_2_LISTVIEW.ListItems(LSTPROCESS_2_LISTVIEW.SelectedItem.Index).SubItems(2)
...
Form2_WORK_SWAPPER.Label16.BackColor = Label12.BackColor
Label29.BackColor = Label12.BackColor

Form2_WORK_SWAPPER.Label17.BackColor = Label22.BackColor
Form2_WORK_SWAPPER.Label18.BackColor = Label22.BackColor
Form2_WORK_SWAPPER.Label20.BackColor = Label22.BackColor
Form2_WORK_SWAPPER.Label21.BackColor = Label22.BackColor

Form2_WORK_SWAPPER.Label19.BackColor = Label45.BackColor
Form2_WORK_SWAPPER.Label28.BackColor = Label45.BackColor

Form2_WORK_SWAPPER.Label26.BackColor = Label49.BackColor
Form2_WORK_SWAPPER.Label27.BackColor = Label49.BackColor
'------------------------------------

'ACTIVE TO GO
'------------------------------------
Form2_WORK_SWAPPER.Label16.BackColor = Label47.BackColor
...
'0&, 90000)
'lRet = MessageBoxTimeout(ME.HWND, "your message", _
'"Timeout MessageBox Test", MB_SETFOREGROUND Or MB_YESNO Or MB_ICONASTERISK, _
'0&, 90000)
'
'Select Case lRet
...
Private Sub MNU_OS_RESTART_Click()
    TIMER_REBOOT_ROUTINE.Enabled = True
    Me.WindowState = vbMinimized
End Sub
...
Private Sub MNU_GOODSYNC_COLLECTION_SCRIPT_RUN_Click()
        Dim FILE_V
        FILE_V = "C:\SCRIPTER\SCRIPTER CODE -- BAT\BAT 59-RUN GOODSYNC SET SCRIPTOR.BAT"
        If FSO.FileExists(FILE_V) = False Then
            MsgBox "FILE NOT EXIST" + vbCr + vbCr + FILE_V
        End If
...
'    MsgBox """" + GetSpecialfolder(38) + "\Microsoft Visual Studio\VB98\VB6.EXE"" """ + CODER_VBP_FILE_NAME_2 + """"
    ...
    If MNU_NOT_MINIMIZE_VALUE = False Then
        Me.WindowState = vbMinimized
        End If     ' ---- TO DOWN AND EN-ABLE EXIT
...
If InStr(COMMAND_LINE, "MAXIMUM") > 0 Then OR_LOGIC = 1
If InStr(COMMAND_LINE, "TASKBAR") > 0 Then OR_LOGIC = 1
If InStr(COMMAND_LINE, "T") > 0 Then OR_LOGIC = 1
If IsIDE = True Then OR_LOGIC = 1
' Me.Visible = True
' DoEvents
If OR_LOGIC = 1 Then
...
Private Sub MNU_TASK_KILLER_NOT_RESPONDER_GENTAL_Click()
    ...
    If MNU_NOT_MINIMIZE_VALUE = False Then
            Me.WindowState = vbMinimized
        End If
    COMMAND_LINE_01 = "C:\SCRIPTER\SCRIPTER CODE -- BAT\BAT 01-NOT RESPONDER KILLER NOT FORCE WAIT.BAT"
...
Private Sub picCrossHair_MouseMove_02()
    ...
    If tPA.y = 0 Or tPA.y < (Me.Top / Screen.TwipsPerPixelY) Then
        ...
        'If RUN_ONCE_PICCROSSHAIR_MOUSEMOVE_DRAGGING_TRUE = False Then
        '    RUN_ONCE_PICCROSSHAIR_MOUSEMOVE_DRAGGING_TRUE = True
...
Sub ChunkCodeOnMouse()
        ...
        If FROM_LISTVIEW = False Then
            ...
            'If 6 - Len(TxtPID.Text) > 0 Then
                PROCESS_PID_STORE_LST_02 = String(6 - Len(TxtPID.Text), "0") + Trim(Str(TxtPID.Text))
            Else
                PROCESS_PID_STORE_LST_02 = ""
            End If
            PROCESS_PID_STORE_LST_03 = PROCESS_PID_STORE_LST_02
        ...
        If InStr(TxtEXE.Text, "\") > 0 Then
            TXTEXE_VAR = TxtEXE.Text
            TXTEXE_VAR = Mid(TXTEXE_VAR, InStrRev(TXTEXE_VAR, "\") + 1)
        End If
        If InStr(UCase(TXTEXE_VAR), "WSCRIPT") > 0 Then
                ' txtTitle.Text = WSCRIPT_SCRIPTNAME(TxtPID.Text)
        End If
        
        TEXT_EXE_FILENAME.Text = FIND_SCRIPTNAME(TxtPID.Text)
...
Sub TIMER_IF_AUTO_HOT_KEY_RUNNER_AND_STOP_THEN_QUIT_HERE_TIMER()
    ...
    '        End If
            PID = -1
            VAR = cProcesses.GetEXEID(PID, "D:\VB6\VB-NT\00_Best_VB_01\TIMEZONE MINI GUI DISPLAY\TIMEZONE MINI GUI DISPLAY.exe")
            If PID <> -1 Then
                VAR = cProcesses.Process_Kill(PID)
                Beep
            End If
...
Sub TIMER_IS_D_DRIVE_GOODSYNC2GO_RUNNER_Timer()
    ...
    If GOODSYNC_WINDOW_HWND > 0 Then
        If FSO.FileExists(PATH_1 + FILE_1) = False Then
...
Sub TIMER_IS_C_DRIVE_GOODSYNC2GO_RUNNER_Timer()

    Dim FILE_NAME, PARAM, TxtEXE_Text_CHECK
    Dim PATH_1, FILE_1, PID_MARK As Long
    Dim GOODSYNC_WINDOW_HWND, Success_Result
    GOODSYNC_WINDOW_HWND = FindWindow("{B26B00DA-2E5D-4CF2-83C5-911198C0F00A}", vbNullString)
    PATH_1 = "C:\SCRIPTOR DATA\VB_KEEP_RUNNER_IS_C_HDD_GOODSYNC2GO_RUNNER\"
    If FSO.FolderExists(PATH_1) = False Then CreateFolderTree (PATH_1)
    FILE_1 = "C_HDD_GOODSYNC2GO_RUNNER_" + GetComputerName + ".TXT"
'    TxtEXE_Text = ""
    On Error GoTo EXIT_SUB
...
'    TxtEXE_Text_CHECK = ""
'    If TxtEXE_Text <> "" Then TxtEXE_Text_CHECK = Mid(TxtEXE_Text, 1, 1)
'    If OLD_TxtEXE_Text_INFO = "C" And TxtEXE_Text_CHECK <> "C" Then
'        ' ---------------------------------------
'        FILE_NAME = "C:\SCRIPTER\SCRIPTER CODE -- BAT\BAT 59-RUN GOODSYNC SET SCRIPTOR.BAT"
'        PARAM = "C_DRIVE_GOODSYNC_JOB_SET"
'        ' ---------------------------------------
'        Dim WSHShell
'        Set WSHShell = CreateObject("WScript.Shell")
'            WSHShell.Run """" + FILE_NAME + """" + " " + "" + PARAM + "", ShowWindow_2, DontWaitUntilFinished
...
'            i = cProcesses.Process_Kill(TEAMVIEWER_PID)
    ...
    ' -----------------------------------------------------------------------
    ' WHEN TEAMVIEWER_FIND = True -- ARRIVE EXIST OWN MACHINE ALL OF A SUDDEN
    ' AAND THEN TELL OTHER MAHCINE PUT IT GO
    ' -----------------------------------------------------------------------
    ...
    ' -------------------------------------------------------
    ' HERE IS SYNC DISTRIBUTE BY COMPUTER 7G DESKTOP GOODSYNC
    ' EACH COMPUTER FIND LAND THERE
    ' -------------------------------------------------------
...
'   ----------------------------------------------------------------
    ...
    TEAMVIEWER_FILE_PRESENT = False
'   ----------------------------------------------------------------
'   HERE IS DELETE AT FORM LOAD
'   BUTTY OTHER COMPUTER MAY HAVE THEM LEFT OVER THING
'   WHEN COMPUTER WANT START TEAMVIEW IT TALKER THE OTHER ONE
'   WHICH TALKER MY COMPUTER AND FIND WITH LOADER
'   WOULD BE HANDY WHEN REMOTE REQUEST COMPUTER TO RUN
'   OUGHT NOT INPUT SCREEN
'   ----------------------------------------------------------------
'   THE ONE FILE CHECKER HERE
'   OUGHT CHECK DATE OF IT OR THROW AWAY
'   ----------------------------------------------------------------
    If FSO.FileExists(PATH_1 + FILE_1) = True Then
        Set F = FSO.GetFile(PATH_1 + FILE_1)
        If F.DateLastModified > Now - TimeSerial(4, 0, 0) Then
            Kill PATH_1 + FILE_1
            Exit Sub
        End If
...
Sub TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNING()
' C:\Program Files\AutoHotkey\AutoHotkey.exe
Dim FIND_AUTOHOTKEY, R1
FIND_AUTOHOTKEY = 0


' 10 MINUTE FROM STARTUP
If TIME_TO_RUNNER_CHECK_AUTOHOTKEY_IS_RUNNER > Now Then
    TIME_TO_RUNNER_CHECK_AUTOHOTKEY_IS_RUNNER = 0 ' --- NOT RUN AGAIN
    Call TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNER_GO
    Exit Sub
End If

If Hour(Now) <> OLD_HOUR_NOW_TO_RUN_AUTOHOTKEY Then
    OLD_HOUR_NOW_TO_RUN_AUTOHOTKEY = Hour(Now)
    ' RUN -------------------------------------------
    Call TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNER_GO
End If
...
Sub TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNER_GO()
    
    Dim R1, FIND_AUTOHOTKEY
    For R1 = 1 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count
        A1 = LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R1).SubItems(2)
        If UCase(A1) = UCase("AutoHotkey.exe") Then
            FIND_AUTOHOTKEY = FIND_AUTOHOTKEY + 1
        End If
    Next
    If FIND_AUTOHOTKEY < 8 Then
        Call Label_TRIPPER_KILL_ALL_RELOAD_AHK_Click
    End If
End Sub
...
Call TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNING
' --------------------------------------------------------------
' WHEN NOT RUN AT BOOT HERE CODE AND AHK NOT RUN LESS THAN VAULE
' OPERATION IN 10 MINUTE FROM BOOTER
' --------------------------------------------------------------
Call TIMER_ONE_HOUR_CHECK_AUTOHOTKEY_IS_RUNNING
' --------------------------------------------------------------
...
If Second(Now) Mod 2 = 0 Then
    ...
    ' Call TIMER_IS_C_DRIVE_GOODSYNC2GO_RUNNER_Timer
...
If MDIProcServ.TT_1VDT > 0 Or MDIProcServ.TT_2VDT > 0 Then
    ...
    DAY_CODE = Format(DateDiff("h", MDIProcServ.TT_1VDT, Now) / 24, "0.00")
    DAY_CODE_HOUR = Format(DateDiff("h", MDIProcServ.TT_1VDT, Now), "0.00")
    ...
    'OS_RE
    If REBOOT_DAY > 0 Then
        MNU_OS_RESTART.Caption = "OS RESTART -- " + Format((REBOOT_DAY * 24) - DAY_CODE_HOUR, "0.00") + " HOUR TO GO"
    End If
    
    YEAR_DIFF = Format(DateDiff("d", MDIProcServ.TT_2VDT, Now) / 365, "0.0") + "Y"
    
    MONTH_DIFF_MOD = Format(DateDiff("m", MDIProcServ.TT_2VDT, Now) Mod 12, "0") + "M"
    YEAR_OF_MONTH_MOD_12 = Trim(Str(Int(DateDiff("m", MDIProcServ.TT_2VDT, Now) / 12))) + "Y"
    Form1.Text_OS_INSTALL_DATE_02.Text = Str(DateDiff("d", MDIProcServ.TT_2VDT, Now)) + "D " + YEAR_DIFF + " " + YEAR_OF_MONTH_MOD_12 + "_" + MONTH_DIFF_MOD
...
If REBOOT_DAY = 0 Then
    If GetComputerName = "1-ASUS-X5DIJ" Then REBOOT_DAY = 2
    If GetComputerName = "2-ASUS-EEE" Then REBOOT_DAY = 2
...
End If
' -------------------------------------------------------------------------
' REBOOT DAY WILL EXTEND ONE DAY WHEN REQUEST TO NOT AGREE BOOT DOWN MSGBOX
' -------------------------------------------------------------------------

If REBOOT_DAY > 0 Then
    MNU_OS_RESTART.Caption = "OS RESTART -- " + Format((REBOOT_DAY * 24) - DAY_CODE_HOUR, "0.00") + " HOUR TO GO"
    If (REBOOT_DAY * 24) - DAY_CODE_HOUR < 0 Then
        TIMER_REBOOT_ROUTINE.Enabled = True
    End If
End If


If DAY_CODE_SHUTDOWN >= REBOOT_DAY And REBOOT_DAY > 0 Then 'And Hour(Now) = 0 Then
    TIMER_REBOOT_ROUTINE.Enabled = True
    ' LET CONTINUE MIGHT HAVE SHUTDOWN THING PROCESS KILLER
    ' -----------------------------------------------------
    ' Sun 26-Jan-2020 20:34:00
    ' Tue 04-Feb-2020 20:48:00
    ' -----------------------------------------------------
End If
...
If InStr(UCase(GetWindowTitle(Me.hWnd)), "NOT RESPONDING") > 0 Then
    X_ONE_SECOND_NOT_RESPOND = X_ONE_SECOND_NOT_RESPOND + 1
        ...
        ' WINDOWS 10 = 5.1
        '----------------------------------------------------
        If GetWindowsVersion_VAR <= 5.1 Then ' XP
    ...
    If X_ONE_SECOND_NOT_RESPOND > DELAY_TIME_NOT_RESPOND Then
        ...
        ' HERE PROGRAM GET RUN AND DISPLAY MESSENGER
        ' ------------------------------------------
...
' STRESS_LEVEL > 4 BIGGER THAN 4 SECOND
...
Sub TIMER_REBOOT_ROUTINE_TIMER()
    ...
    ' -------------------------------------------------
    ' NOT WORK VB6
    ' Wed 23-Sep-2020 20:32:35
    ' -------------------------------------------------
    TIMER_REBOOT_ROUTINE.Enabled = False
    TIMER_REBOOT_ROUTINE.Interval = 25000 ' 25 SECOND
    ...
    RESULT = MessageBoxTimeout(Me.hWnd, MSGBOX_STRING, App.EXEName + ".EXE", MB_SETFOREGROUND Or MB_YESNO Or MB_ICONASTERISK, 0&, 10000&)
    If RESULT = IDNO Then
        REBOOT_DAY = REBOOT_DAY + 1
        Exit Sub
    End If
    If 1 = 1 Or RESULT = IDYES Then
        ' GIVE 1 MINUTE QUESTION NOT PUT ONE AFTER OTHER
        ' ----------------------------------------------
        REBOOT_DAY = REBOOT_DAY + TimeSerial(0, 1, 0)
        ' Exit Sub
    End If
    Dim WSHShell
    Set WSHShell = CreateObject("WScript.Shell")
        If IsIDE = False Then WSHShell.RUN "C:\WINDOWS\SYSTEM32\SHUTDOWN.EXE /R /T 40", 0, True
    Set WSHShell = Nothing
    If MNU_NOT_MINIMIZE_VALUE = False Then
        Me.WindowState = vbMinimized
    End If
...
Dim HWND_CABINETWCLASS_E_HDD_HARDWARE

HWND_CABINETWCLASS_E_HDD_HARDWARE = FindWindow("CabinetWClass", vbNullString)

If HWND_CABINETWCLASS_E_HDD_HARDWARE = 0 Then
    Exit Sub
End If
If OLD_HWND_CABINETWCLASS_E_HDD_HARDWARE = HWND_CABINETWCLASS_E_HDD_HARDWARE Then
'    Exit Sub
End If

OLD_HWND_CABINETWCLASS_E_HDD_HARDWARE = HWND_CABINETWCLASS_E_HDD_HARDWARE
On Error Resume Next
...
ALLOW_GO = False
If O_DAY_NOW_MIDNIGHT_HARDWARE <> Int(Now) Then ALLOW_GO = True
O_DAY_NOW_MIDNIGHT_HARDWARE = Int(Now)
ALLOW_GO = True
...
If ALLOW_GO = True Then
    ...
    End If
End If
If ALLOW_GO = False Then Exit Sub

If FSO.FolderExists(DIR_PATH + "\HARDWARE " + Format(Now, "YYYY-MM-DD -")) = True Then
     Exit Sub
End If
...
For r = 2 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count
    A1 = LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(r).SubItems(2)
    A2 = LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(r - 1).SubItems(2)
    If A1 = A2 Then
        ' IF TWO ARE RUNNER
...
' --------------------------------------------
If OLD_Timer_Pause_Update_Enabled <> Timer_Pause_Update.Enabled Then
    If Timer_Pause_Update.Enabled = True Then
        ENUMPROCESS_MUST_RUNNER = True
        ENUMPROCESS_NOT_RUN_YET = False
    End If
End If
OLD_Timer_Pause_Update_Enabled = Timer_Pause_Update.Enabled
...
' RATHER THAN DEL CLEAR
' OVERWRITE AND CLEAR END SPACE
' Sun 09-Feb-2020 22:20:01
' ----------------------------------------
...
LSTPROCESS_2_LISTVIEW.ListItems.Clear
LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Clear
lstProcess_4_SORT_PID.ListItems.Clear
lstProcess_8_NATURAL.ListItems.Clear
WE_MAX = 0
For R_I = 0 To lstProcess.ListCount - 1
...
'    frmMain.lstProcess_3_SORTER_ListView.ListItems.Add , , ITEM_ADD_10 + " " + ITEM_ADD_22
    ...
    '-----------------------------------------------------
    On Error Resume Next
'    LSTPROCESS_2_LISTVIEW.ListItems.Item(WE) = ITEM_ADD_10
'    LSTPROCESS_2_LISTVIEW.ListItems.Item(WE).SubItems(2) = ITEM_ADD_21
    If 1 = 1 Or Err.Number > 0 Then ' SCRIPT TOO FULL TO EDIT OVER ITEM AND THEN ADD MORE
        WE_MAX = -2
'        DO LATER
'        -----------------
'        With LSTPROCESS_2_LISTVIEW
'            Set LV2 = .ListItems.Add(, , ITEM_ADD_10)
'            LV2.SubItems(2) = ITEM_ADD_21
'            '------------------------
'            'PAIN TO GET THE FORMULAR
'            'EVEN THEN FAR OUT AGAIN
'            '------------------------
...
'If LSTPROCESS_2_LISTVIEW.ListItems.Count > WE_MAX And WE_MAX > -2 Then
'    For R_I = lstProcess.ListCount - 1 To WE_MAX Step -1
'        LSTPROCESS_2_LISTVIEW.ListItems.Remove (R_I)
'    Next
'End If
...
O_lstProcess_ListCount = lstProcess.ListCount
CURRENT_LSTPROCESS_LISTCOUNT = lstProcess.ListCount
...
'If LSTPROCESS_2_LISTVIEW.SortKey <> 1 Then GO_X = 1
'If LSTPROCESS_2_LISTVIEW.SortOrder <> lvwAscending Then GO_X = 1
'If LSTPROCESS_2_LISTVIEW.Sorted = True <> True Then GO_X = 1

If lstProcess_4_SORT_PID.SortKey <> 1 Then GO_X = 1
If lstProcess_4_SORT_PID.SortOrder <> lvwAscending Then GO_X = 1
If lstProcess_4_SORT_PID.Sorted = True <> True Then GO_X = 1

' DO SORT WHEN THING CHANGE CONSTANT EVER
If GO_X <> 0 Then
    lstProcess_4_SORT_PID.SortOrder = lvwAscending
    lstProcess_4_SORT_PID.SortKey = 0
    lstProcess_4_SORT_PID.Sorted = True

'    LSTPROCESS_2_LISTVIEW.SortOrder = lvwAscending
'    LSTPROCESS_2_LISTVIEW.SortKey = 0
'    LSTPROCESS_2_LISTVIEW.Sorted = True
'    'lstProcess_3_SORTER_ListView.Sorted = False
End If
...
Sub POPULATE_THE_LIST_VIEW()
    ...
    Dim LV2 As ListItem ' NOT CONFUSE __ LISTVIEW WITH __ LISTITEM __ MUST BE __ ListItem
    LSTPROCESS_2_LISTVIEW.ListItems.Clear
    If LSTPROCESS_2_LISTVIEW_COLUMNCLICK_VAR = True Then
        For R_I = 1 To lstProcess_4_SORT_PID.ListItems.Count
            ITEM_ADD_10 = lstProcess_4_SORT_PID.ListItems(R_I)
            ITEM_ADD_21 = lstProcess_4_SORT_PID.ListItems(R_I).SubItems(1)
            ITEM_ADD_22 = lstProcess_4_SORT_PID.ListItems(R_I).SubItems(2)
            With LSTPROCESS_2_LISTVIEW
                Set LV2 = .ListItems.Add(, , ITEM_ADD_10)
                LV2.SubItems(2) = ITEM_ADD_22
            End With
            LSTPROCESS_2_LISTVIEW.ColumnHeaders.Item(3).Text = "EXE - PID SORT"
        Next
    Else
        For R_I = 1 To lstProcess_8_NATURAL.ListItems.Count
            ITEM_ADD_10 = lstProcess_8_NATURAL.ListItems(R_I)
            ITEM_ADD_21 = lstProcess_8_NATURAL.ListItems(R_I).SubItems(1)
            ITEM_ADD_22 = lstProcess_8_NATURAL.ListItems(R_I).SubItems(2)
            With LSTPROCESS_2_LISTVIEW
                Set LV2 = .ListItems.Add(, , ITEM_ADD_10)
                LV2.SubItems(2) = ITEM_ADD_22
            End With
            LSTPROCESS_2_LISTVIEW.ColumnHeaders.Item(3).Text = "EXE - PID NATURAL SORTER"
        Next
    End If
    Set LV2 = Nothing
    ...
    If UPDATE_O_NOW_LV_AUTOSIZE_1 = True Then
        ...
        Call LV_AutoSizeColumn(LSTPROCESS_3_SORTER_LISTVIEW)
        LSTPROCESS_3_SORTER_LISTVIEW.ColumnHeaders.Item(2).width = 220
        
        Call LV_AutoSizeColumn(LSTPROCESS_2_LISTVIEW)
        LSTPROCESS_2_LISTVIEW.ColumnHeaders.Item(2).width = 220
    End If
...
Sub PROCESS_LISTVIEW_2_AND_3_ENSURE_VISIBLE_SELECTOR_STAY()
        ...
        For R_I = 1 To LSTPROCESS_2_LISTVIEW.ListItems.Count ' - 1
            If LSTPROCESS_2_LISTVIEW.ListItems(R_I) = PROCESS_PID_STORE_LST_02 Then
                ...
                If LSTPROCESS_2_LISTVIEW.ListItems(R_I) <> LSTPROCESS_2_ENSUREVISIBLE_VALUE Then
                    TINDEX = LSTPROCESS_2_ENSUREVISIBLE_INDEX
                    On Error Resume Next
                        ' SOMETIME WHEN TOP ONE DELETER -- INDEX OUT OF BOUND
                        LSTPROCESS_2_LISTVIEW.ListItems(TINDEX).SubItems(1) = ""
                    On Error GoTo 0
                End If
                End If
    ...
    If PROCESS_PID_STORE_LST_03 > -1 Then
        ...
        For R_I = 1 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count '  - 1
            If LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I) = PROCESS_PID_STORE_LST_03 Then
                ...
                LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).EnsureVisible
                ' lstProcess_3_SORTER_ListView.ListItems.ITEM
                LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).Selected = True
                CHK_VAR_OBTAIN_3 = 1
                
                If LSTPROCESS_3_ENSUREVISIBLE_INDEX > 0 Then
                If LSTPROCESS_3_ENSUREVISIBLE_INDEX <> R_I Then
                If LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I) <> LSTPROCESS_3_ENSUREVISIBLE_VALUE Then
                    TINDEX = LSTPROCESS_3_ENSUREVISIBLE_INDEX
                    LSTPROCESS_3_SORTER_LISTVIEW.ListItems(TINDEX).SubItems(1) = ""
                End If
                End If
                End If
    ...
    If PROCESS_PID_STORE_LST_02 > -1 Then
        OLD_PROCESS_PID_STORE_LST_02 = PROCESS_PID_STORE_LST_02
    End If
    If PROCESS_PID_STORE_LST_03 > -1 Then
        OLD_PROCESS_PID_STORE_LST_03 = PROCESS_PID_STORE_LST_03
    End If
    
    If CHK_VAR_OBTAIN_2 = 0 Then
        PROCESS_PID_STORE_LST_02 = -1
    End If
    If CHK_VAR_OBTAIN_3 = 0 Then
        PROCESS_PID_STORE_LST_03 = -1
    End If
    If PROCESS_PID_STORE_LST_02 > -1 And CHK_VAR_OBTAIN_2 = 1 Then
        PROCESS_PID_STORE_LST_02 = -1
    End If
    If PROCESS_PID_STORE_LST_03 > -1 And CHK_VAR_OBTAIN_2 = 1 Then
        PROCESS_PID_STORE_LST_03 = -1
    End If

    Dim ISP_1
    Dim ISP_2
    ISP_1 = GetScrollPos(LSTPROCESS_2_LISTVIEW.hWnd, 1)
    ISP_2 = GetScrollPos(LSTPROCESS_3_SORTER_LISTVIEW.hWnd, 1)
    ' Debug.Print "SCROLL BAR 1 & 2 " + vbCr + Str(ISP_1) + vbCr + Str(ISP_2)
...
PROCESS_TO_KILLER = LSTPROCESS_2_LISTVIEW.ListItems(1).SubItems(2)
T_COUNTER = 1
For R_I = 0 To LSTPROCESS_2_LISTVIEW.ListItems.Count - 1
    If R_I > 0 Then
        PROCESS_TO_KILLER = LSTPROCESS_2_LISTVIEW.ListItems(R_I).SubItems(2)
...
PROCESS_TO_KILLER = LSTPROCESS_3_SORTER_LISTVIEW.ListItems(1).SubItems(2)
T_COMPARE = PROCESS_TO_KILLER
T_COUNTER = 1
For R_I = 0 To LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Count - 1
    If R_I > 0 Then
        PROCESS_TO_KILLER = LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I).SubItems(2)
...
If LSTPROCESS_3_SORTER_LISTVIEW_MOUSEDOWN_BUTTON = 2 Then
    
    MNU_NOT_MINIMIZE_VALUE = True
    Call MNU_NOT_MINIMIZE_SET
    If MNU_NOT_MINIMIZE_VALUE = False Then
        Me.WindowState = vbMinimized
    End If
    
    LSTPROCESS_3_SORTER_LISTVIEW_MOUSEDOWN_BUTTON = 0
    Dim R_I
    R_I = LSTPROCESS_3_SORTER_LISTVIEW.SelectedItem.Index
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).Selected = True
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).EnsureVisible
    ...
    R_I = LSTPROCESS_3_SORTER_LISTVIEW.SelectedItem.Index
    A_TO_Z_PROCESS_NAME = LSTPROCESS_3_SORTER_LISTVIEW.ListItems(R_I).SubItems(2)
    
    Set objShell = CreateObject("Wscript.Shell")
    objShell.RUN "TASKKILL /F /IM """ + A_TO_Z_PROCESS_NAME + """", 0, True
    Set objShell = Nothing
        
    Dim R_LOOP
    Dim OLD_CURRENT_LSTPROCESS_LISTCOUNT
    For R_LOOP = 1 To 20
        ENUMPROCESS_MUST_RUNNER = True
        Call EnumProcess
        If CURRENT_LSTPROCESS_LISTCOUNT < OLD_CURRENT_LSTPROCESS_LISTCOUNT Then
            Exit For
        End If
        OLD_CURRENT_LSTPROCESS_LISTCOUNT = CURRENT_LSTPROCESS_LISTCOUNT
    
    Next
    
    R_I = R_I + 1 ' SELECT NEXT ONE AFTER PROCESS KILL ITEM GONE
    ' OR UNDER LATER CODE
    ' LATER
    ' Fri 26-Jun-2020 02:27:28
    ' Fri 26-Jun-2020 01:19:36 -- 1 HOUR 8 MINUTE
    ' AND EARLIER
    
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).Selected = True
    LSTPROCESS_3_SORTER_LISTVIEW.ListItems.Item(R_I).EnsureVisible
    
    Call lstProcess_3_SORTER_ListView_Click
End If
...
If TxtEXE_Text = "" Then
    ...
    If TxtEXE_Text = "" Then
        ...
        ' -----------------------------------------------------
        ' LESS QUICKER USER -- GetObject("winmgmts:").ExecQuery
        ' NOT TWICE WHEN FIRST DON'T
        ' MOSTLY NOT GET 64 BIT APP PATH -- GUESS
        ' -----------------------------------------------------
...
'txtMemoryUsage_HERE
    On Error GoTo ENDER
    Clipboard.Clear
    Clipboard.SetText Form2_WORK_SWAPPER.txtMemoryUsage
    Exit Sub
ENDER:
    DoEvents
    Sleep 100
