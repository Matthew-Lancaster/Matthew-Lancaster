...
Private inHdr(1 To NUM_BUFFERS) As WaveHdr
'
  ...
  For i = 1 To NUM_BUFFERS
    ...
    If inHdr(k).dwFlags And WHDR_DONE Then
      ...
      If Writing Then
        ...
        If FindWindow("tty", "lame") = 0 And _
            FindWindow("ConsoleWindowClass", _
            App.Path & "\lame.exe") = 0 Then
          MsgBox "Can't find lame anymore"
          StopWrite
          Exit Sub
        End If
        
        Debug.Print "Will write to pipe. "; hWritePipe; sBuff(1); BufferSize
        
        ' This function locks up everything in W2k but works in W98
        rc = WriteFile(hWritePipe, sBuff(1), BufferSize, _
                        NumBytesDone, 0&)
        Debug.Print "Have written to pipe. "; rc
