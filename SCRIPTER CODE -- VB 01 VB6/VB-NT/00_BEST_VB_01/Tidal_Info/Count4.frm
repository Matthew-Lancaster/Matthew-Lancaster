...
'Public CHD, MHD, NHD
Public BlockCountDupe
Public LK2, OldDate4 ', FreeMem, IIO2, IIO3
...
TTCount7 = Val(Mid$(R2$, 1, 4))
If TTCount7 > 0 Then
OldDate4 = DateValue(Mid$(R2$, 16, 9)) + TimeValue(Mid$(R2$, 26, 9))
End If

'If r2$ <> "" Then
'    TTCount5 = Val(Mid$(r2$, 1, 4))
'    OldDate3 = DateValue(Mid$(r2$, 16, 9))
    TTCount8 = Val(Mid$(R2$, 6, 5))
'    If Int(Now) <> DateValue(Mid$(r2$, 16, 9)) Then TTCount5 = 0: OldDate3 = Int(Now)
'End If

Label10.Caption = Trim(str(TTCount7))
...
Public Sub WriteCounter()
    ...
    TTCount7 = TTCount7 + 1
    TTCount8 = TTCount8 + 1
    Label10 = Trim(str(TTCount7))
    If Menu.MuteAllMal = vbChecked Then fg$ = fg$ + " -- Mute All Mal"

    QQ$ = Format$(TTCount7, "0000 ") + Format$(TTCount8, "00000-") + Format$(Now, "DDD-DD-MMM-YY HH:MM:SSa/p ") + "-" + fg$
    'List1.AddItem qq$
    ...
    Counter4.BackColor = Label5.BackColor
    Timer4.Enabled = True
...
hwndw = FindWindow(vbNullString, Counter3.Caption)
If hwndw = 0 Then Exit Sub
Dim Rect4 As RECT
Hwnd24 = GetWindowRect(hwndw, Rect4)
On Local Error Resume Next
If Ik2 <> Rect4.Top Then
    Counter4.Top = (Rect4.Bottom * Screen.TwipsPerPixelX)
    Counter4.Left = (Counter3.Left) '- Counter.Width)
End If
Ik2 = Rect4.Top
End Sub
...
Counter4.BackColor = Label6.BackColor
