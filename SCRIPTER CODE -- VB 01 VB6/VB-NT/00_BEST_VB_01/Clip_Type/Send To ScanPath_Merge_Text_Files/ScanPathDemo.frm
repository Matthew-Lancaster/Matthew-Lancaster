...
txtPath.Text = "D:\# MY DOCS\# 01 My Documents\01 BlueTooth SMS's From CLipType"
If Right$(txtPath.Text, 1) <> "\" Then txtPath.Text = txtPath.Text + "\"
txtpathlen = Len(txtPath)

cboMask.Text = "*.txt;*.nfo;*.diz;*.html,*h,*.c,*ccp,*.rc,*.dsp,*.log,*.bas;*.cls;*.frm;*.nws;*.vcf"
...
FF$ = "## Blue #01 TOTAL.txt"
'Kill txtPath.Text + FF$
...
For WE = 1 To ListView1.ListItems.Count
    ...
    If Len(A1) <> txtpathlen Then
        FD = Mid$(A1$, txtpathlen + 1)
        FD = "-" + Mid$(FD, 1, InStr(FD, "\") - 1)
    Else
        FD = " #02 ROOT"
    End If
    If xfd <> FD Then
        ...
        XTag = 0
        FF$ = "## Blue" + FD + ".txt"
'        If Dir(txtPath.Text + FF$) <> "" Then Kill txtPath.Text + FF$
        FR1 = FreeFile
...
'fr2 = FreeFile
'Open txtPath.Text + FF$ For Append As #fr2
...
Fs22.copyFile txtPath + FF$, "D:\# MY DOCS\# 01 My Documents\00 Email Texts\Dir to Text List\" + FF$
...
If WE >= ListView1.ListItems.Count Then Exit Sub
...
Fs22.deletefolder Drived2$ + "Temp\Favs\*"
...
Fs22.deletefolder txtPath.Text + "\*"

'MkDir txtPath.Text

Fs22.Movefolder Drived2$ + "Temp\Favs\*", txtPath.Text
