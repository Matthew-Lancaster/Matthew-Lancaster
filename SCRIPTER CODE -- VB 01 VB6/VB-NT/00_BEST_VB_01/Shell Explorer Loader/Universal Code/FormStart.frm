...
For R3 = 0 To UBound(NET_PATH_ALL_R)
    If NET_PATH_ALL_R(R3) <> "" Then
        ...
        For R5 = 1 To 5
            ...
            If InStr(CK1, "9_ASUS") > 0 Then STRV = 2
            STRV = STRV - 1
            ...
            If InStr(CK2, "9-A") > 0 And InStr(CK2, "MATT 01") > 0 Then SET_HERE_2 = True
...
' ------------------------------------------------------
' ADD HEADER TITLE FOR EXPLORER SHELL FOLDER -- 01 OF 02
' ------------------------------------------------------
With ScanPath.ListView1
    Set LV = .ListItems.Add(, , "\EXPLORER SHELL FOLDER 01 TO 02\")
    LV.SubItems(1) = "TITLE_BLOCK"
End With
Dim ARRAY_V()
Dim ARRAY_V_2()
...
AVI = AVI + 1: ARRAY_V(AVI) = "SHELL:DOWNLOADS"
...
AVI = AVI + 1: ARRAY_V(AVI) = "shell:AppsFolder"
...
For R_AVI = 1 To UBound(ARRAY_V)
    ARRAY_V(R_AVI) = Replace(ARRAY_V(R_AVI), "shell:", "SHELL:")
Next
AVI = 0
For R_AVI = 1 To UBound(ARRAY_V)
    If InStr(STRING_COMPARE, ARRAY_V(R_AVI)) = 0 Then
    If ARRAY_V(R_AVI) <> "" Then
        AVI = AVI + 1
        ARRAY_V_2(AVI) = ARRAY_V(R_AVI)
        STRING_COMPARE = STRING_COMPARE + ARRAY_V(R_AVI) + "__"
    End If
    End If
Next
...
' ------------------------------------------------------
' ADD HEADER TITLE FOR EXPLORER SHELL FOLDER -- 02 OF 02
' ------------------------------------------------------
With ScanPath.ListView1
    Set LV = .ListItems.Add(, , "\EXPLORER SHELL FOLDER 02 OF 02\")
    LV.SubItems(1) = "TITLE_BLOCK"
End With
'Dim ARRAY_V()
'Dim ARRAY_V_2()
...
'AVI = AVI + 1: ARRAY_V(AVI) = "shell:AppData"
AVI = AVI + 1: ARRAY_V(AVI) = ""
AVI = AVI + 1: ARRAY_V(AVI) = ""
AVI = AVI + 1: ARRAY_V(AVI) = ""
AVI = AVI + 1: ARRAY_V(AVI) = ""
...
With ScanPath.ListView1
    Set LV = .ListItems.Add(, , "\GOODSYNC PROFILE FOLDER\")
    LV.SubItems(1) = "TITLE_BLOCK"
End With
...
For R3 = 0 To UBound(M)
    If Dir(M(R3)) <> "" Then
        ...
        With ScanPath.ListView1
            ...
            Set LV = .ListItems.Add(, , UCase(TT1))
            LV.SubItems(1) = Path
...
For R1 = ScanPath.ListView1.ListItems.Count To 0 Step -1
    R2 = R1 + 1
    R2 = R2 + 1
    ' ScanPath.ListView1.ListItems.Item(R1).SubItems(2)
    If ScanPath.ListView1.ListItems.Item(R1) = ScanPath.ListView1.ListItems.Item(R2) Then
    If ScanPath.ListView1.ListItems.Item(R1).SubItems(1) = ScanPath.ListView1.ListItems.Item(R2).SubItems(1) Then
    If ScanPath.ListView1.ListItems.Item(R1) = ScanPath.ListView1.ListItems.Item(R3) Then
    If ScanPath.ListView1.ListItems.Item(R1).SubItems(1) = ScanPath.ListView1.ListItems.Item(R3).SubItems(1) Then
        ScanPath.ListView1.ListItems.Remove (R1)
    End If
    End If
    End If
    End If
...
For R1 = ScanPath.ListView1.ListItems.Count To 0 Step -1
    If Trim(Mid(ScanPath.ListView1.ListItems.Item(R1), 1, 1)) = "" Then
        ScanPath.ListView1.ListItems.Item(R1) = Trim(ScanPath.ListView1.ListItems.Item(R1))
    End If
Next
...
' THIS VARIABLE CAME AS ERRRO PROCEED HAPPEN WHEN FIRST CAME
' AND THEN NOT NEXT ROUTINE
If SetTrueToLoadLast = True Then
...
If D1$ <> "" Then
    ...
    SET_GO = True
    ' SEEM GOT SYSTEM ONLY RUN ONCE IN WHILE
    ' --------------------------------------
    If COMPUTER_NAME_PUT_STORE_NETWORK_2_STEP_JUMPER_01 = "" Then SET_GO = False
    If COMPUTER_NAME_PUT_STORE_NETWORK_2_STEP_JUMPER_02 = "" Then SET_GO = False
    I41 = COMPUTER_NAME_PUT_STORE_NETWORK_2_STEP_JUMPER_01
    ...
    If InStr(I45, "D:\VIDEO") > 0 Then
        ...
        If Form1.MNU_NET_2_GO.Caption = "[__ NET 2 GO = GO __]" Then
            ...
            End If
        End If
        If Form1.MNU_NET_2_GO.Caption = "[__ NET 2 GO __]" Then
            Form1.MNU_NET_2_GO.Caption = "[__ NET 2 GO NOP__]"
            Exit Sub
        End If
    End If
    End If
    ...
    If SET_GO = False Then
        ...
        End If
    End If
    If OVERIDE_IF_SET_GO_FALSE = False Then
    If Form1.MNU_NETWORK_2_STEP_DRIVE_SELECTOR.Visible = False Then
        ' WORK UNDERWAY UNFINSIH PART HAD ERROR WANT BEHAVIOUR
        ' INVERT STATE SOUND REASON
        ' MODIFY AFTER HERE ADD VIDEO GO -- WHEN EXTRA 2 STEP DO
        ' Thu 28-May-2020 15:08:42
        SET_GO = False
    End If
    End If
...
Public Sub EXPLORER_WITH_SHELL(SELECT_OPTION, FOLDER_NAME)
    ...
    If InStr(FOLDER_NAME, "SHELL:") > 0 Then
        
        ' Shell "CMD " + FOLDER_NAME, vbMaximizedFocus
        ' Call SHELL_CMD(FOLDER_NAME, "CMD /C """ + FOLDER_NAME + """")
        
        'vLaunch "CMD /C """ + FOLDER_NAME + """", vbMaximizedFocus
'        Dim objShell
'        Set objShell = CreateObject("Wscript.Shell")
'        objShell.Run "CMD /K " + FOLDER_NAME, 0, True
'        Set objShell = Nothing
