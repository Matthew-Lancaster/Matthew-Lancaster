...
Begin VB.Form Form1 
   ...
   End
   Begin VB.Menu MNU_SEND_TO_DATE_MODIFY 
      Caption         =   "SEND TO DATE MODIFY"
   End
   ...
   End
   Begin VB.Menu MNU_MOVE_HERE_FILE_CORRECT_LOCATION 
      Caption         =   "MOVE HERE FILE CORRECT LOCATION"
   End
   ...
   End
   Begin VB.Menu MNU_VB_SEND_TO_TOUCHDATE 
      Caption         =   "VB SEND TO DATE MODIFY"
   End
   ...
   End
   Begin VB.Menu MNU_HASH_MY_FILES_ONE_FOLDER 
      Caption         =   "HASH MY FILES ONE FOLDER"
   End
   Begin VB.Menu MNU_MULTI_MEDIA_CONVERTOR 
      Caption         =   "MULTI MEDIA CONVERTOR"
   End
   Begin VB.Menu MNU_DEL_EMPTY 
      Caption         =   "DEL EMPTY"
   End
   Begin VB.Menu MNU_GET_ALL_EXTENSION_TYPE 
      Caption         =   "GET ALL EXTENSION TYPE"
   End
...
'fr1 = FreeFile
'Open App.Path + "\" + App.EXEName + ".exe" For Binary Access Read Lock Read As #fr1
...
If Label_1_FOLDER_FILE.Caption <> "" Then
    If InStr(GetLongName(Label_1_FOLDER_FILE.Caption), "M4H") > 0 Then
        Call MNU_FILEDATE_CLIPBOARD_Click
    End If
End If
...
Private Sub MNU_XnView_MP_Click()
    Me.Hide
    ...
    If OSI = 64 Then
        HASH_EXE = "C:\Program Files\XnViewMP\xnviewmp.exe"
    Else
        HASH_EXE = "C:\Program Files X(86)\XnViewMP\xnviewmp.exe"
    End If
    
    WSHShell.Run """" + HASH_EXE + """ """ + XF0 + """"
...
Private Sub MNU_FILE_NAME_TO_TEXT_FILE_Click()
Call FILE_NAME_TO_TEXT_FILE
End Sub
...
Sub MNU_FILEDATE_CLIPBOARD_ROUTINE_CHECKER()
        ...
        TEE = "Photo"
        If InStr(GetLongName(FILE_NAME), TEE) > 0 Then
            If InStr(GetLongName(FILE_NAME), ".JPG") > 0 Then
                START_MAQ_LEN = 7
                MAQ = Mid(FILE_NAME_2, InStr(FILE_NAME_2, TEE), START_MAQ_LEN)
                START_MAQ = InStr(FILE_NAME_2, TEE)
            End If
...
Private Sub MNU_GET_ALL_EXTENSION_TYPE_Click()
...
If GetComputerName = "4-ASUS-GL522VW" Then
    Label_2_FOLDER.Caption = "D:\VIDEO\NOT\X 00 NOT"
End If
...
Private Sub MNU_MEDIA_PLAYER_CLASSIC_LINE_CLIPBOARD_CLIPPATH_Click()

' WORK
' Thu 30-Jan-2020 21:50:51
' Thu 30-Jan-2020 22:58:00 -- 1 HOUR 7 MINUTE

EXE_LOADER = "C:\Program Files\MPC-HC\mpc-hc64.exe"

RENAME_COPY_FILE = "C:\TEMP\IRFAN_SLIDESHOW__MPC.TXT"
RENAME_COPY_FILE = Replace(RENAME_COPY_FILE, ".TXT", ".M3U")

XD = Clipboard.GetText
If XD = "" Then
    Beep
    Exit Sub
End If

If InStr(XD, vbCr) > 0 Then
    XH = Mid(XD, 1, InStr(XD, vbCr) - 1)
End If
If Dir(XH + "K") <> "" Then
Else
    MsgBox "FIRST LINE TEXT NOT INCLUDE A FILENAME THAT FOUND" + vbCrLf + "WILL USER LAST TEXTFILE BEFORE IF EXIST" + vbCrLf + vbCrLf + "CLIP TESTER ITEM " + vbCrLf + XH + vbCrLf + "CLIP ITEM WHOLE " + vbCrLf + XD
    If Dir(RENAME_COPY_FILE) <> "" Then
        GOT_TEXTFILE_BEFORE = True
    End If
    If GOT_TEXTFILE_BEFORE = "" Then
        Exit Sub
    End If
End If
...
If GOT_TEXTFILE_BEFORE = "" Then
    FR1 = FreeFile
    Open RENAME_COPY_FILE For Output As #FR1
        Print #FR1, XD
    Close FR1
End If
...
If Dir(EXE_LOADER) <> "" Then
    On Error Resume Next
    Shell EXE_LOADER + " """ + RENAME_COPY_FILE + """", vbMaximizedFocus
    If Err.Number > 0 Then
        MsgBox "ERROR LOADING THE SHELL " + vbCrLf + VBCLRF + EXE_LOADER, vbMsgBoxSetForeground
    End If
End If
...
Private Sub MNU_MOVE_HERE_FILE_CORRECT_LOCATION_Click()

' "C:\DOWNLOADS\stylus-2020-03-03.json"
    
FILE_NAME_1 = Label_1_FOLDER_FILE.Caption
...
If InStr(FILE_NAME_1, ".json") Then
    ...
    If FSO.FileExists(DESTI) = True Then
        ' REQUIRE RENAME
        Set F = FS.GetFile((FILE_NAME_1))
        ADATE1 = F.DateLastModified
    End If
    If FSO.FileExists(DESTI) = True Then
    For r = 0 To 100
        FILE_NAME_2 = "stylus-" + Format(ADATE1, "YYYY-MM-DD__HH-MM-SS") + ".json"
        DESTI = "C:\SCRIPTER\SCRIPTER CODE -- USERSTYLES.ORG STYLISH\" + FILE_NAME_2
        If FSO.FileExists(DESTI) = True Then
            ADATE1 = ADATE1 + TimeSerial(0, 0, r)
        Else
            Exit For
        End If
    Next
    ...
    FSO.CopyFile FILE_NAME_1, DESTI
    Shell "EXPLORER /SELECT, """ + DESTI + """", vbNormalFocus
    If FSO.FileExists(DESTI) = False Then
        MsgBox "FILE NOT MOVE OKAY __" + vbCrLf + "SOURCE AND DESTINATION DISPLAY EXPLORER__" + vbCr + vbCrLf + "SOURCE ONLY DELETE IF AFTER COPY OKAY OPERATION MOVE__" + vbCrLf + vbCrLf + FILE_NAME_1 + vbCrLf + vbCrLf + DESTI
        Shell "EXPLORER /SELECT, """ + FILE_NAME_1 + """", vbNormalFocus
End If
...
Private Sub MNU_MULTI_MEDIA_CONVERTOR_Click()
...
If OSI = 64 Then
    MMC_EXE = "C:\Program Files (x86)\MIKSOFT\Mobile Media Converter\mmc.exe"
Else
    MMC_EXE = "C:\Program Files\MIKSOFT\Mobile Media Converter\mmc.exe"
End If

WSHShell.Run """" + MMC_EXE + """ /folder """ + XF0 + """"
...
Private Sub MNU_SEND_TO_DATE_MODIFY_Click()

' D:\VB6\VB-NT\00_Send_To\Send To TouchDate\#Send To TouchDate.exe

Call MNU_VB_Send_To_TouchDate_Click
...
PID = Shell("C:\HBCD\Programs\Programs\NTFSAccess2.4.exe", vbNormalFocus) ' vbMaximizedFocus)
If PID > 0 Then Beep: Unload Me
...
Dim PARAM
PARAM = """" + Label_1_FOLDER_FILE.Caption + ""

Dim WSHShell
Set WSHShell = CreateObject("WScript.Shell")
    WSHShell.Run """" + "D:\VB6\VB-NT\00_Best_VB_01\#0 SMALL PROGS\Create on another drive\Create Dir Another Drive.exe" + PARAM
Set WSHShell = Nothing
...
End Sub
Private Sub MNU_VB_Send_To_TouchDate_Click()
...
Dim WSHShell
Set WSHShell = CreateObject("WScript.Shell")
    WSHShell.Run """" + "D:\VB6\VB-NT\00_Send_To\Send To TouchDate\#Send To TouchDate.exe " + PARAM
Set WSHShell = Nothing
...
Private Sub MNU_HASH_MY_FILES_ONE_FOLDER_Click()
    ...
    ' USE /FILE -- It Do Folder Also Term It as Filename
    ' --------------------------------------------------------
    WSHShell.Run """" + HASH_EXE + """ /file """ + XF0 + """"
...
Private Sub FILE_NAME_TO_TEXT_FILE()

CMD_LINE = Label_1_FOLDER_FILE

If CMD_LINE = "" Then
    'Me.Show
    DoEvents
    
    MsgBox "NOT FILE OR FOLDER" + vbCrLf + CMD_LINE + vbCrLf + Command$, vbMsgBoxSetForeground
End If
