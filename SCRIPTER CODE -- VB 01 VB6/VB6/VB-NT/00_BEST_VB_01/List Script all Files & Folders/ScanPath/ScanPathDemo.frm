...
Private Sub SP_FileMatch(Filename As String, DFilename As String, Path As String, dtFileDate1 As Date, dtFileDate2 As Date, dtFileDate3 As Date, dtFSize As Long)
    ...
    'On Local Error GoTo GetFileError
        ...
        End If
       'GETS FILESIZE BUT NOT DATE
    
    
'    If MNU_IRFAR_MODE_SET = True Then
''        Print #FF01, Path + Filename
'        Call FRM_SEND_TO.XSCRIPT(Path + Filename)
'        Exit Sub
'    End If
    ...
    'WFD.ftCreationTime    As FILETIME
    'ftLastAccessTime  As FILETIME
    'ftLastWriteTime   As FILETIME
    
    'ADATE1 = FormatFileDate(uWIN32.ftCreationTime)
    'ADATE1 = FormatFileDate(uWIN32.ftCreationTime)
    'ADATE1 = FormatFileDate(uWIN32.ftLastWriteTime)
    'ADATE1 = FormatFileDate(uWIN32.ftLastWriteTime)
    
    'PROBLEM FINDING THE DATE
    ...
    DAT2 = STRING2 + Format(dtFileDate2, "YYYY-MM-DD HH-MM-SS")
    
    STRING2 = "  Date Last Accessed:-"
    DAT3 = STRING2 + Format(dtFileDate3, "YYYY-MM-DD HH-MM-SS") '+ vbCrLf
            
    
    'FindFilesAPISize = (uWIN32.nFileSizeHigh * MAXDWORD) + uWIN32.nFileSizeLow
    
    'DAT4 = Format((uWIN32.nFileSizeHigh * MAXDWORD) + uWIN32.nFileSizeLow, "000,000,000,000,000")
    
    DAT4 = Format(dtFSize, "000,000,000,000,000")
    
    
    Call frm_SEND_TO.NAUGHT_NULL(DAT4, 0, -1)
    DAT4 = "Size:-" + DAT4 + " :--"
...
'
    ...
    Call frm_SEND_TO.NAUGHT_NULL(DATN0, 0, -1)
    ...
    Print #FF01, "Alternative Name :- "; DAT5; " -- File"
    Print #FF01, DAT4;
    'Print #FF01, DATN0; " - ";
    'Print #FF01, Str((uWIN32.nFileSizeHigh * MAXDWORD) + uWIN32.nFileSizeLow)
    Print #FF01, Str(dtFSize)
    
    'Print #FF01, DAT5;
    
    Print #FF01, DAT1; DAT2; DAT3
    Print #FF01, "<-------------------->"
...
Private Sub SP_DirMatch(Directory As String, DDirectory As String, Path As String, dtFileDate1 As Date, dtFileDate2 As Date, dtFileDate3 As Date)
    '####################################################################################################
    'This Event fires for each Folder found
    '####################################################################################################
    
    If MNU_IRFAR_MODE_SET = True Then Exit Sub
...
'    DAT4 = " - "
    ...
    If DDirectory = "" Then
        MsgBox "STOP BECAUSE A DIRECTORY EMPTY NAME" + vbCrLf + Directory + vbCrLf + DDirectory + vbCrLf + Path
        If IsIDE = True Then Stop
    End If
...
'
    ...
    'USING FILE COUNT - WE DON'T WANT THAT
    DATN0 = Format(mDIRCount2 + mFILECount2, "000,000,000,000,000")
    Call frm_SEND_TO.NAUGHT_NULL(DATN0, 0, -1)
    DATN0 = DATN0 ' + " -"
    ...
    Print #FF01, "Alternative Name :- "; DAT5; " -- Directory"
    
    'Print #FF01, DAT4 - SIZE BUT ALWAYS NAUGHT
    'Print #FF01, DATN0; " - "; Str((uWIN32.nFileSizeHigh * MAXDWORD) + uWIN32.nFileSizeLow)
    'Print #FF01, DAT5;
    Print #FF01, DAT1; DAT2; DAT3
    Print #FF01, "<-------------------->"
...
Private Sub SP_DirMatchxx(Directory As String, DDirectory As String, Path As String, dtFileDate1 As Date, dtFileDate2 As Date, dtFileDate3 As Date)
    '####################################################################################################
    'This Event fires for each Folder found
    '####################################################################################################
...
For Each Control In Controls
        'CONTROL.NAME
    If Mid(UCase(Control.Name), 1, 3) <> "MNU" Then
...
On Error Resume Next ' PROBLEM WITH UNLOADING
    Me.Top = 500
    'If FRM_SEND_TO.Enabled = True Then
    '    Me.Top = FRM_SEND_TO.Top
    'End If
On Error GoTo 0
...
'Private Function FormatFileDate2(CT As FILETIME) As String
'
'    Const SHORT_DATE = "Short Date"
'
'    Dim ST As SYSTEMTIME
...
'------------------ NOT SURE ABOU THIS BIT
    ...
    frm_SEND_TO.LabelX(3) = Trim(Str(mFILECount2) + " -- File")
    frm_SEND_TO.LabelX(4) = Trim(Str(mDIRCount2) + " -- Folder")
    
    ScanPath.lblCount1 = Format(mFILECount3, "###,###,###,###,###,###,0") + " -- File - Total"
    ScanPath.lblCount2 = Format(mDIRCount3, "###,###,###,###,###,###,0") + " -- Folder - Total"
    
    frm_SEND_TO.LabelX(5) = ScanPath.lblCount1
    frm_SEND_TO.LabelX(6) = ScanPath.lblCount2
    ...
    ScanPath.lblCount3 = Format(COUNTPROCES2, "###,###,###,###,###,###,0") + " " + " -- File and Folder " 'YCH '"  PROCESS"
    'ScanPath.lblCount3 = Format(COUNTPROCES3, "###,###,###,###,###,###,0") + " " + " Folders and Files Total" 'YCH '"  PROCESS"
    frm_SEND_TO.LabelX(7) = ScanPath.lblCount3
    frm_SEND_TO.LabelX(8) = Format(COUNTPROCES3, "###,###,###,###,###,###,0") + " " + " -- File and Folder - Total" 'YCH '"  PROCESS"
    
    i4 = DateDiff("s", PROCESSBEGIN, Now)
    i5 = DateDiff("s", PROCESSBEGIN2, Now)  ' PER DRIVE
    i7 = DateDiff("n", PROCESSBEGIN, Now)
    i8 = DateDiff("n", PROCESSBEGIN2, Now)  ' PER DRIVE
    
    'ScanPath.lblCount5 = Format(i4, "0.000") + " MIN"
    frm_SEND_TO.LabelX(9) = Trim(Str(i8)) + " Minutes -- " + Trim(Str(i5 Mod 60)) + " Seconds -- Per Drive Logger"
    If LastDriveTime <> "" Then
        frm_SEND_TO.LabelX(9) = frm_SEND_TO.LabelX(9) + " --- Last Drive Was - " + LastDriveTime
    End If
    frm_SEND_TO.LabelX(10) = Trim(Str(i7)) + " Minutes -- " + Trim(Str(i4 Mod 60)) + " Seconds -- Total Logger --" + Str(COUNTPROCES5) + " File and Folder Per Second --" + Str(COUNTPROCES9) + " Per Minute"
    
    ScanPath.lblCount4 = frm_SEND_TO.LabelX(9)
    
    'FRM_SEND_TO.LabelX(9) = ScanPath.lblCount5
    ...
    frm_SEND_TO.LabelX(11) = "FILE   -- " + Replace(MDIR2, "\", "\ ")
    frm_SEND_TO.LabelX(12) = "PATH -- " + Replace(MDIR3, "\", "\ ")
