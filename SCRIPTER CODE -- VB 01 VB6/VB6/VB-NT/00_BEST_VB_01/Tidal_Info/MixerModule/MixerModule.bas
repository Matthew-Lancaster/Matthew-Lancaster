...
Public Function SetVolume(Control As VOL_CONTROL, ByVal NewVolume As Long) As Long
    ...
    'If NewVolume = 100 Then Stop
    If Control <> SPEAKER Then Exit Function
    
    SetVolume = SetControlValue(hMixerHandle, uMixerControls(Control), NewVolume)
    ATidalX.LblVol = str(NewVolume)
    
    'FR4 = FreeFile
    'Open App.Path + "\00_Text_Data\Volume.txt" For Output As #fr4
