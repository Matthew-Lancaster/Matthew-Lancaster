...
'Function QEmptyRecycleBinSize(ByVal uObject As Object) As String
Function QEmptyRecycleBinSize() As String
    
    Dim RBinInfo As SHQUERYRBINFO, msg As VbMsgBoxResult
    RBinInfo.cbSize = Len(RBinInfo)
    SHQueryRecycleBin vbNullString, RBinInfo
    STRINGRE = "Recycle Bin Lad"
    
    If (RBinInfo.i64Size.lowpart And &H80000000) = &H80000000 Or RBinInfo.i64Size.highpart > 0 Then
        'msg = msgbox("Your Recycle Bin consumes over 2 GB right now!" + vbCrLf + "Do you want to empty it?", vbYesNo + vbQuestion)
        QEmptyRecycleBinSize = STRINGRE + " Is Over 2 Gig ¤"
    Else
        'msg = msgbox("Your Recycle Bin consumes" + str$((RBinInfo.i64Size.lowpart) / 1024) + " KB right now." + vbCrLf + "Do you want to empty it?", vbYesNo + vbQuestion)
        QEmptyRecycleBinSize = STRINGRE + " ¤ " + Format((RBinInfo.i64Size.lowpart) / 1024, "0.0") + "K"
        
        If RBinInfo.i64Size.lowpart / 1024 >= 1000 Then
        'RBinInfo.i64Size.lowpart
        QEmptyRecycleBinSize = STRINGRE + " ¤ " + Format((RBinInfo.i64Size.lowpart) / 1024 ^ 2, "0.0") + "M"
        If RBinInfo.i64Size.lowpart = 0 Then
            QEmptyRecycleBinSize = STRINGRE + "en ¤ Zero"
        End If
        If RBinInfo.i64Size.lowpart > 1024 ^ 3 Then
            QEmptyRecycleBinSize = STRINGRE + " ¤ " + Format((RBinInfo.i64Size.lowpart) / 1024 ^ 3, "0.0") + "G"
        End If
...
End Function
Private Function Round(Value As Double, Digits As Integer) As Double
