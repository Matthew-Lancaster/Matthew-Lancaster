...
TitleLbl.Top = x
TitleLbl.Left = 0
TitleLbl.Height = Check1.Height + 50
...
TitleLbl.Width = Form1.Width / 2 - 15
Check1.Left = 0 'Form1.Width - (Form1.Width / 2)
...
If tq = 0 Then MsgBox "Not Right Path Not Found in shortcut Prog Launch Explorer"
rr$ = Mid$(rr$, tq - 1)
End If
...
If tth > 0 Then
a1$ = ScanPath.ListView2.ListItems.Item(tth).SubItems(1)
b1$ = ScanPath.ListView2.ListItems.Item(tth)
a1$ = a1$ + b1$
End If
Skip:
If tth = 0 Then a1$ = rr$
On Local Error Resume Next
ChDir a1$
If Err.Number > 0 Then MsgBox a1$ + vbCrLf + "Dir Dont Exist" + vbCrLf + Err.Description: End

Shell "D:\VB6\VB-NT\00_Best_VB_04\JPEG-Encoder-ART\Jpeg Slides PJpeg ART.exe """ + a1$ + """", vbNormalFocus
'Shell "C:\Program Files\Microsoft Visual Studio\VB98\VB6.EXE /r D:\VB6\VB-NT\00_Best_VB_01\JPEG-Encoder\pjpeg.vbp /cmd """ + a1$ + """", vbNormalFocus
End
'-------------- Mod
...
t = ScanPath.ListView1.ListItems.Count
If t = 0 Then tth = tth + 1:
Loop Until t > 0
xxr = 1
End If
...
Do
tth = ScanPath.ListView2.ListItems.Count
If tth > 0 Then tth = Int(Rnd(1) * (ScanPath.ListView2.ListItems.Count + 1))
...
t = ScanPath.ListView1.ListItems.Count
tthx = ScanPath.ListView2.ListItems.Count
If t = 0 And tthx = 0 Then MsgBox "No Pix files End": End
...
a1$ = ScanPath.ListView1.ListItems.Item(xxr).SubItems(1)
b1$ = ScanPath.ListView1.ListItems.Item(xxr)
