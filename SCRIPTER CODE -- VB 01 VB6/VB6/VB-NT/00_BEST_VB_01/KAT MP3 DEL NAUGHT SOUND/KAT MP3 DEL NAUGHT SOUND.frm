...
Begin VB.Form Form1 
   ...
   ScaleHeight     =   5925
   ScaleWidth      =   12075
   Begin VB.Timer Timer_SHUTDOWN_ONCE_A_DAY_PROBLEM_HOGGING 
      Interval        =   1000
      Left            =   7185
      Top             =   1335
   End
...
Dim ODAY_VAR_SHUTDOWN_PROBLEM_APPS_PIC_PICK
Dim ODAY_VAR_SHUTDOWN_PROBLEM_APPS
...
Private Sub Timer_SHUTDOWN_ONCE_A_DAY_PROBLEM_HOGGING_Timer()

If ODAY_VAR_SHUTDOWN_PROBLEM_APPS = Day(Now) Then Exit Sub
ODAY_VAR_SHUTDOWN_PROBLEM_APPS = Day(Now)

Dim PID As Long

'PID HAS TO BE -1 AT ENTRY
PID = -1
Var = cProcesses.GetEXEID(PID, "C:\PROGRAM FILES\PICPICK\PICPICK.EXE")
...
Shell "C:\PROGRAM FILES\PICPICK\PICPICK.EXE"

If Err.Number > 0 Then
    '-------------------------------
    'IF ERROR SOMETIMES HAPPEN RETRY
    '-------------------------------
    ODAY_VAR_SHUTDOWN_PROBLEM_APPS = -2
    ODAY_VAR_SHUTDOWN_PROBLEM_APPS_PIC_PICK = ODAY_VAR_SHUTDOWN_PROBLEM_APPS_PIC_PICK + 1
    
    If ODAY_VAR_SHUTDOWN_PROBLEM_APPS_PIC_PICK > 10 Then
        MsgBox "PROBLEM RUN -- " + vbCrLf + "C:\PROGRAM FILES\PICPICK\PICPICK.EXE" + vbCrLf + "RETRY ATTEMPT = >10 -- ABANDON RETRY FOR A DAY" + vbCrLf + Err.Description, vbMsgBoxSetForeground
        ODAY_VAR_SHUTDOWN_PROBLEM_APPS = Day(Now)
        ODAY_VAR_SHUTDOWN_PROBLEM_APPS_PIC_PICK = 0
    End If
...
If FIRST_RUN_FIND_PID_KAT_MP3 = False Then
    ...
    If PID = -1 Then
        ...
        If Dir(KAT_MP3_PATH) = "" And TAG_MSG_DONE = False Then
            TAG_MSG_DONE = True
            MsgBox KAT_MP3_PATH + vbCrLf + "DON'T EXIST"
        Else
            TAG_MSG_DONE = False
        End If
        ...
        If IsIDE = False Then Shell KAT_MP3_PATH, vbMinimizedNoFocus
        COUNT_UP = Now
