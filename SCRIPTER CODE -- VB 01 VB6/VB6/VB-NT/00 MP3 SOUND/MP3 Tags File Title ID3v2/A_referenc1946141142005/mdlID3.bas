...
Private Function ReadV2Tag(ByVal fh As Long, ByVal fp As Long, ByRef RdTag As ID3Tag, ByVal bToMem As Boolean) As Long
    ...
    If TagSizes.ReadSize > 0 Then
    ReDim TagData(TagSizes.ReadSize - 1)
    Else
    ReDim TagData(TagSizes.ReadSize)
    End If
...
'Public Function ReadID3v2(ByVal FileName As String, ByRef outTag As ID3Tag) As Boolean
Public Function WriteID3v2(ByVal fileName As String, ByRef InTag As ID3Tag, Optional ByVal Version As ID3v2_Version, Optional ByVal bClearExistingTag As Boolean, Optional ByVal bIgnoreReadOnly As Boolean = True, Optional ByVal pBuffer As Long = 0, Optional ByVal BufferSize As Long = 0) As Boolean
...
'    If InStr(fileName, "iss2") > 0 Then Stop
    
    bClearExistingTag = False
    Version = VERSION_2_3
    ...
    If fh = INVALID_HANDLE_VALUE Then
        'MsgBox (fileName + vbCrLf + "File Got a INVALID_HANDLE_VALUE" + vbCrLf + "Proberbly -- File is containing Just a Header and no MP3 data no Use to Us")
        Exit Function
    End If
...
'    If InStr(fileName, "iss2") > 0 Then Stop
    ...
    Else
        ...
        Set f = Fs22.Getfile(fileName)
        Tt = f.Size
        If Tt > fsize Then
            fsize = Tt
        End If
        
        'File is containing Just a Header and no MP3 data no Use to Us
        If fsize = ExistSize Then
            'MsgBox (fileName + vbCrLf + "File is containing Just a Header and no MP3 data no Use to Us")
            Exit Function
        End If
...
Private Function Data2Long(ByVal pData As Long, ByVal bSynchSafe As Boolean) As Long
    ...
    Else
        'Mod by Matthew 2009-06-28 error resume
...
'                If Not opByte = 0 And Not opByte = 15 Then
                ...
                'Modded Bye Matt
                If opByte = 15 Then outInfo.Bitrate = outInfo.Bitrate / 10
                
                If outInfo.Bitrate > 0 Then outInfo.Length = (fsize - fp) / (outInfo.Bitrate * 125)
                If outInfo.Bitrate = 0 Then outInfo.Length = (fsize - fp)
